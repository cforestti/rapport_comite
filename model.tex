...
Dans ce document, on s'intéresse par les principaux modèles afin d'identifier et l'adapter celui qui s'adapte le mieux à l'essai DCLT.
...
Le modèle proposé par \cite{Smith1960} constitue une grande avancée à la compréhension du phénomène mais reste très limité. D'autres modèles ont tenté de modéliser le phénomène à l'aide de paramètres caractérisants le sol pénétré \cite{Randolph2003ScienceDesign, Loukidis2008AssessmentProcedure}.
...
Soit G le module de cisaillement du sol en Pascal, R le rayon de la semelle en mètres et $\nu$ le coefficient de Poisson. Le coefficient d'amortissement est donné par l'équation \ref{eq:CLys}. Pour les sables non-saturé, \cite{Whitman1969} indique des valeurs entre 0,35 et 0,40 pour $\nu$.
