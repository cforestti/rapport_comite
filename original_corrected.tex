...
Dans le cas des pénétromètres dynamiques (SPT, DCP, ...), le principal paramètre fourni reste le nombre de coup par enfoncement. Même si ceci sert comme indicateur de la résistance au cisaillement, ce paramètre ne permet pas d'interpréter rationnellement d'essai, car il le traduit par l'interaction du système marteau-tige-sol \citep{Schnaid2017InterpretationSoils}.

Dans l’état actuel des connaissances, l’essai de chargement dynamique en pointe pénétrométrique (DCLT, de l’anglais \emph{Dynamic Cone Load Test}) réalisé à l’aide du pénétromètre $Panda 3\up{\textregistered}$\footnote{Désormais noté Panda 3} \citep{Benz-Navarrete2009MESURES2, Escobar2015Mise3} permet de répondre à un grand nombre des besoins (coût, rapidité, haut rendement de mesures). Grâce à l’instrumentation et l’exploitation des mesures de force et d’accélération au niveau de tête de l’appareil, il est possible d’estimer les grandeurs cinématiques à l’interface pointe-sol, à savoir : force, vitesse, déplacement.
...
Il est représenté par une loi élastoplastique parfaite avec une composante visqueuse de type newtonienne basée sur modèle masse-ressort-amortisseur de Smith (1962). Bien qu'intéressant, ce modèle ne permet pas caractériser le sol à l'aide de paramètres rhéologiques couramment appliqués en géotechnique.
...
La détermination du module élastique du matériau est plus difficile, car ceci dépend de nombreux facteurs (granulométrie, état de densité, confinement, histoire de chargement, niveau de déformation, ...) \cite{Briaud2001INTRODUCTIONMODULI}. Pour les sables propres, principaux facteur sont l'état de densité, la pression de confinement et le niveau de déformation \cite{Hardin1972ShearEffects}.

Du fait du comportement non-linéaire des sols, à partir de la courbe contrainte-déformation issue de l'essai triaxial, on peut définir différents modules. On s'intéresse principalement aux modules sécants, c'est-à-dire, les modules calculés sur le rapport contrainte-déformation pour une déformation donnée de la courbe relié à l’origine. Ces modules sont adaptés principalement pour prédire le tassement d’un sol de fondation dû à l’application d’une charge (apportée par une semelle, par exemple) \citep{Briaud2001INTRODUCTIONMODULI}. 

Parmi eux, les modules les plus courants dans la pratique sont le module tangent initial ($E_{ini}$) (théoriquement équivalent à un module sécant pour la déformation nulle) et le module sécant à 50\% de la contrainte de rupture ($E_{50}$).

La détermination du module initial à partir de résultats triaxiaux s'avère assez délicate car pour des faibles déformations, on constate des fluctuations importantes pour ce paramètre. Ceci est aussi rapporté par \cite{Lancelot1996ComportementDraines} sur ses résultats triaxiaux pour de sable d'Hostun. Par conséquent, on choisit de retenir le module sécant à déformation axiale 0,1\% comme le module initial \citep{Lancelot1996ComportementDraines,Dano2001ComportementInjectes}. En effet, la résolution de l'essai triaxial classique, en général, ne permet pas de déterminer des modules à de déformations inférieures à $10^{-3}$ \citep{Obrzud2018TheGuidebook}.

Il faut préciser que le terme << initial >> employé fait référence à courbe contrainte-déformation issue de l'essai triaxial. On ne prétend pas de déterminer le module élastique qui est, à la rigueur, un paramètre associé à des déformations axiales de l'ordre de $10^{-5}$. On est conscient que cela est impossible avec un matériel et un protocole classique de l'essai triaxial \citep{Dupla1995ApplicationSable}.
...
Il est courant d'admettre que l'angle de frottement de pic des sols granulaires dépend de la densité et l'état de contrainte \citep{Rouse2018RelationTest,Lancelot1996ComportementDraines}. Toutefois, l'influence de l'état de contrainte étant plus prononcée plus de contraintes faibles (< 100 kPa). Comme les essais triaxiaux présentés ont été réalisés à des pressions de confinement égales ou supérieures à 50 kPa, on suppose que la variation de $\phi'$, pour la gamme de confinement analysée, est négligeable. Ainsi, on présente les résultats en fonction uniquement de l'indice de vides (e). 
...
Or, on constate que nos résultats, comme ceux rapportés, sont plus élevés pour le sable d'Hostun que pour le sable de Fontainebleau. Cela peut être expliqué en partie par l'angularité des grains, le sable d'Hostun présentant des grains anguleux à sub-anguleux, tandis ceux de sable de Fontainebleau sont plutôt arrondis \citep{Aris2012ExperimentalElements, Benahmed2004InitialSand}.
  ...
  \begin{figure}[H]
   \begin{center}
        \includegraphics[scale=.9]{media/exp_frictionangle4.PNG}
...
Pour des faibles contraintes (< 100kPa), à la rigueur, les graphes p-q ne sont pas linéaire, ceci indique une variation de l'angle de frottement à la rupture avec la pression moyenne. Il est généralement admis que l'angle de frottement $\phi'$ à la rupture décroît quand le niveau de contrainte augmente, toutefois les travaux divergents quand à cette variation (Ponce et Bell, 1971 ; Kolymbas et Wu (1990), \cite{Lancelot1996ComportementDraines, Maksimovic1989NONLINEARSOILS}.

Notamment pour le sable d'Hostun, cet effet a été démontré par d'autres travaux (Institut de Mécanique de Grenoble par Flavigny en communication personnelle aux membres du GRECO et \citep{Lancelot1996ComportementDraines}).

Ces travaux relatent une décroissance sensible est régulière de $\phi'$ avec l'augmentation de la pression de confinement, cet effet étant plus prononcé pour des contraintes entre 20 kPa et 100 kPa \citep{Lancelot1996ComportementDraines}.

Des corrélations ont été proposés par certains auteurs afin de reproduire l'évolution de $\phi'$ en fonction de la contrainte moyenne p'. Ainsi une relation hyperbolique à trois paramètres a été proposée par \citep{Maksimovic1989NONLINEARSOILS}.

\begin{equation}
    \phi' = \phi_B + \frac{\Delta \phi}{1+\frac{p'}{p_N}}
\end{equation}

Les paramètres sont l'angle de frottement de base $\phi_B$, l'angle maximal de variation $\Delta \phi$ et la pression moyennement $p_N$ déterminés à partir des résultats triaxiaux pour un sol donné.
...
La pénétration dynamique reste un des problèmes les plus sophistiqué des applications numériques en géotechnique. Il s’agit d’un problème fortement non-linéaire et qui entraîne des déformations très importantes \citep{Nazem2012a}.
...
La méthode explicite utilisée dans FLAC est associée à une analyse lagrangienne. Les coordonnées des noeuds du maillage dans l’espace correspondent à des particules du milieu dont les positions sont référencées par rapport à la configuration initiale. Ainsi, la fonctionnalité dite « en grandes déformations » permet de modéliser des problèmes à grandes déformations car les coordonnées des noeuds sont mises à jour à chaque pas de temps. Cette méthode s’oppose à la méthode eulérienne où le matériau de déplace et se déforme tandis que le maillage est fixe. Selon \cite{Ghee2011}, un des avantages de la méthode intégrée dans FLAC constitue le qu’il fonctionne en petites ainsi qu’en grandes déformations. Dans la fonctionnalité dite en « grandes déformations », le maillage est mis à jour lors de déformations ou de mouvements. Ce mode est particulièrement intéressant pour traiter les problèmes à grandes déformations comme c’est le cas d’un essai de pénétration.
...
Étant donné le caractère axisymétrique du problème (conditions aux limites, chargement vertical centré, matériau homogène) et dans le but réduire le temps de calcul, on souhaite simuler uniquement un quart de la géométrie. Pour ce faire, il s'avère nécessaire vérifier si cette simplification entraîne des changements non-négligeable dans les résultats. Afin de vérifier cela, on a simulé l'essai DCLT pour chacune des géométries (cas a et cas b, \ref{fig:num_axy}).

Pour cette analyse, le massif possède les mêmes caractéristiques dans les simulations : homogène élasto-plastique parfait (E=54,4 MPa, phi=38,85\degree, c=7 kPa, dil=8,85\degree). Les conditions aux limites sont  dans un premier temps avec la géométrie intégrale et dans un deuxième temps avec 1/4 de la géométrie. On présente sur la Figure \ref{fig:num_axy} les courbes issues de ces calculs. Celles-ci montrent que les résultats reste assez proches, par conséquent on choisit d'utiliser désormais la géométrie axisymétrique.

\begin{figure}[H]
\begin{center}
    \includegraphics[scale = .8]{media/num_axy}
    \caption{Comparaison des courbes contrainte déplacements pour les deux géométries}
      \label{fig:num_axy}
  \end{center}
\end{figure}
...
Celles-ci sont constituées d’amortisseurs dans les deux directions perpendiculaires aux noeuds de éléments de sol placés aux limites (bords latérales) \citep{Lysmer1969}. Ces amortisseurs apportent des forces normales et transversales ($t_n$ et $t_s$). Ces forces sont fonction des vitesses normales et transversales ($v_n$ et $v_s$), de la masse volumique $\rho$ et des célérités d’ondes ($c_p$ et $c_s$) des éléments placés aux limites. L'application de ces forces aux noeuds placés sur les faces limitant le modèle permet d'empêcher la réflexion des ondes aux bords du modèle. 
...
La qualité du maillage est essentielle pour la précision et fiabilité des résultats. Un grand défi pour la modélisation continue de la pénétration est le fait d’entraîner de grandes déformations dans le sol. Les limitations quant aux déformations du maillage varient selon la méthode intégrée dans le code. En général, le code associant la méthode Lagrangienne et la méthode Eulérienne permettent de mieux gérer les déformations (\emph{e.g.} code Abaqus). La méthode Lagrangienne intégrée par le code FLAC permet de mettre à jour les coordonnées des noeuds au cours du calcul. Cette méthode n’est pourtant pas adaptée lorsque les déformations deviennent trop importantes \citep{Moug2019, Nagula2018}.
...
Différentes tailles d’élément de maillage ont été testées afin d’analyser la sensibilité des résultats. Le but étant de trouver un maillage suffisamment fin afin de converger vers une solution unique sans toutefois trop augmenter le temps de calcul. Le modèle est composé par 3048 éléments et 4664 noeuds.
...
Selon \cite{Ahmadi2005a}, ce critère de rupture simple est capable de traduire correctement le comportement des sables. Les auteurs l’appliquent afin de modéliser le comportement d’un sable purement frottant (comme le sable de Ticino). L’angle de frottement est obtenu à partir d’une série d’essais triaxiaux tandis que l’angle de dilatance est estimé à partir de l’angle de frottement effectif $\phi$’ et l’angle de frottement à l’état critique $\phi_{cv}$.
...
En supposant la variation de $\phi'$ avec le confinement est négligeable pour les pressions de confinement appliquées ($\le$100 kPA), on a tracé l'évolution de $\phi$ en fonction de l'état de densité de trois éprouvettes testées à l'appareil triaxial. Le but étant d'estimer l'angle de frottement correspondant à l'état de densité des éprouvettes réalisées (e = 0,68 et e = 0,78, voir Figure \ref{fig:exp_eprouvettes}). La relation entre l'indices de vides et l'angle de frottement est présentée dans la Figure \ref{fig:exp_phi_SH_SF_tx}. Les valeurs de $\phi'$ obtenues sont : 30,8\textdegree{} et 35,9\textdegree{} pour les éprouvettes lâches et moyennement dense, respectivement.

L’angle de dilatance $\psi$ est quant à lui généralement compris entre 0\degree et 15\degree. Les sables lâches et les argiles ont des valeurs de dilatance très faibles, quelques degrés à peine voire zéro. Il est souvent déterminé en laboratoire à partir de résultats de l'essai triaxial. Il existe également des relations empiriques simples reliant l’angle de dilatance et l’angle de frottement interne \citep{Levasseur2007}. De manière similaire à \cite{Ahmadi2005Thin-layerMeasurement}, on estime l'angle de dilatance à partir de l'angle de frottement en rupture et l'angle de frottement critique (Équation \ref{eq:dilatance}). Comme évoqué, les valeurs rapportées dans la littérature pour le $\phi_{cr}$ varient peu. Celles-ci sont comprises entre 29,3\textdegree{} et 31,0\textdegree{} \citep{Dano2001, Dupla2007, Vernay2017, Andria-Ntoanina2011}.
...
L'étude bibliographique réalisée à propos des modèles d'interaction a permis de souligner leur points convergeants et leurs limitations. A l'issue de cette étude, on envisage d'appliquer un modèle d'interaction sol-pointe plus réaliste (comme à celui de \cite{Loukidis2008}) afin de dissocier la composante dynamique et de pouvoir appliquer des modèles d'interprétation comme l'expansion de cavité à cette technique. 
...
\emph{Oliveira, C. F. ; Lopez, S. R. ; Tran, Q. A. ; Breul, P. ; Canou, J. ; Reiffsteck, P. ; Dupla, J. C. ; Jacquard, C. ; Teyssier, A. ; Benz-Navarrete, M. A. Soil modulus assessment by means of cone dynamic loading test. 20th International Conference on Soil Mechanics and Geotechnical Engineering (ICSMGE), Sydney, May 2022. (Présentation orale - Résumé accepté)}\\[0.01cm]

\emph{Oliveira, C. F. ; Benz-Navarrete, M. A. ; Breul, P. ; Tran, Q. A. ; Chevalier, B. ; Bacconnet, C. Assessment of small-strain modulus through wave velocity measurement with dynamic penetrometer Panda 3®. 6th International Conference on Geotechnical and Geophysical Site Characterization, Budapest, Septembre 2021. (Présentation orale)}
