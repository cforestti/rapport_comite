...
La démarche appliquée pour déterminer $c_p^{DCLT}$ est celle des essais de choc. Les essais de choc sont réalisés en laboratoire avec des éprouvettes de petite taille. \cite{Omidvar2012, IskanderMaguedOmidivar2015} relatent que la caractérisation de sols à partir des essais de choc est plus complexe que pour d'autres matériaux (comme les métaux par exemple). Pour des essais de choc pour les matériaux de vitesse d'ondes plus élevés une distribution de contraintes dans l'échantillon est plus uniforme. Dans le cas de matériaux granulaires comme les sables, la vitesse de propagation n'étant pas très élevés, l'échantillon n'atteint pas l'équilibre pour des chargements rapides. D'après les auteurs cela est un aspect essentiel pour l'obtention des paramètres.

Afin de garantir une déformation plus uniforme pour le début du chargement, la vitesse de chargement est contrôlée afin d'avoir une augmentation de contrainte quasi-uniforme. D'ailleurs pour les matériaux peut cohésifs (comme les sables) les éprouvettes doivent être confinés avant le chargement dynamique. Souvent un tubage est utilisé afin d'assurer un confinement latérale, même si certains travaux ont montré qui cela influe sur les résultats (Junyu et al., 2014; Lu et al., 2009; Song et al., 2009). D'autres travaux essaient de mettre en place un système triaxial afin de mieux étudier l'effet du confinement.

Une suggestion serait d'utiliser le modèle numérique afin d'étudier l'influence de la forme de l'onde et de conditions aux limites.
