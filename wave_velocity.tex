...
Les essais de propagations d’ondes sont, toutefois, peu courant dans la pratique. Cela induit à des estimations de déformations peu réalistes dans les dimensionnements et constitue une limitation à l'application des modèles de comportement plus poussés (alimentés par les paramètres à faibles déformations).

L'essai de chargement dynamique en pointe pénétrométrique (DCLT) au pénétromètre dynamique léger Panda 3$\up{\textregistered}$  est un technique simple et à faible coût. Lors des premiers instants suivant l'impact, cet essai est assimilable à un essai de choc \citep{Aussedat1970, Semblat2012, IskanderMaguedOmidivar2015}. Comme un essai de choc, l’exploitation des signaux de force et de vitesse dans le sol permet d’étudier le comportement à faibles déformations et d'estimer la vitesse d’ondes de compression ($c_p^{DCLT}$).

Il reste évaluer dans quelle mesure $c_p^{DCLT}$ correspondrait à la vitesse d'ondes de compression $c_p$ issue des techniques connues. Dans ce document on compare les résultats de $c_p^{DCLT}$ et $c_p$ sous des conditions semblables. La vitesse d'ondes de compression issue de à l'essai DCLT étant par hypothèse de compression, les valeurs de $c_p$ présentés sont issues techniques déterminant $c_p$ (aucune estimation à partir de $c_s$).

Les comparatifs se focalisent uniquement sur les résultats obtenus en laboratoire pour les sables propres de référence (sable d'Hostun HN31 et de Fontainebleau NE34). Le choix de ces sables permettent de profiter des différents résultats rapportés dans la littérature pour ces matériaux.
...
\cite{Benz-Navarrete2009} a appliqué la méthode proposée par \cite{Aussedat1970}, à l'essai DCLT. Il a testé différents matériaux (béton, bois et différents sols) en laboratoire. Les polaires de choc obtenues sont répétables et sensibles aux matériaux. Les valeurs de $c_p^{DCLT}$ obtenus sont de l'ordre de grandeur conforme aux valeurs rapportées pour ces matériaux.
...
Quant à la l’état hydrique, ceci est important pour distingué l'état saturé du non saturés (S<95\%). Pour les sables non saturé, ce facteur est secondaire car il n'engendre que des variations de l’ordre de 10\% \citep{Emerson2005}. Tous les comparatifs analysés dans ce document correspond à l'état non-saturé.
...
Parmi des résultats de $c_p$ rapportées dans la littérature, on compare ceux correspondant aux conditions (DR et $\sigma_v$) égales ou le plus proche possible de celles des essais DCLT (Table \ref{tab:comparatifs_}). La Figure \ref{fig:comparatifs_graphique_} présente les données sur forme graphique.


\begin{table}[H]
    \begin{center}
        \includegraphics[scale = .45]{media/comparatifs_.PNG}
        \caption{Comparatifs pour le SF et SH}
        \label{tab:comparatifs_}
  \end{center}
\end{table}

On constate que les valeurs de $c_p^{DCLT}$ sont supérieures à ceux figurant dans la littérature. La seule éprouvette pour laquelle la différence reste inférieure à 15\% étant SF à DR=50\% et contrainte de 50 kPa. On constante également que $c_p^{DCLT}$ est sensible à la densité. Pour les résultats SH à la contrainte de 50 kPa, on observe l'augmentation de $c_p^{DCLT}$ avec la densité. Les écart-types ont été moins importants pour les éprouvettes denses à des confinements importants. Cela est en accord avec les constatations d'\cite{Emerson2005}. Il relate de dispersion importantes les valeurs de $c_p$ obtenus à faibles confinements (en surface).
...
Toutefois, on a pu constaté que $c_p^{DCLT}$ est sensible à la densité et au confinement des éprouvettes et que toutes les valeurs obtenues restent globalement dans la gamme attendu pour des sables sèches (Figure \ref{fig:cp}, \citep{Cassan1988}). 
...
Étant donné les difficultés relatées pour les essais de choc dans les sols, il paraît pertinent d'étudier dans un premier temps l'influence de la forme de l'onde, temps de monte de l'onde (matériau et géométrie du marteau, ...). Dans un deuxième temps, l'influence de la taille de l'échantillon et des conditions aux limites afin de mieux répondre aux hypothèses initiales (déformations radiales négligeables, surface plane normale à la vitesse de propagation à l'interface sol-pointe ).
ontact entre le sol et la pointe pendant la transmission de l'onde est supposé plan et perpendiculaire
