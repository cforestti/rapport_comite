\textbf{\documentclass[12pt]{report}
...
D’une part, pour les premières instantes suites au choc, cet essai de chargement dynamique en pointe pénétrométrique (DCLT) est assimilable à un essai de choc. L’exploitation de forces et de vitesses entraînées dans le sol au cours de cet intervalle permet d’étudier le comportement à faibles déformations et de déterminer une mesure de vitesse d’ondes du sol. 

Par ailleurs, le type de sollicitation, la géométrie et les conditions aux limites de l’essai se ressemblent à un essai dynamique de pieu. De manière similaire à un essai de chargement dynamique des pieux, la réponse du sol en pointe peut être exprimée à l’aide d’un modèle d’interaction. Actuellement, le modèle appliqué à cet essai est le modèle masse-ressort-amortisseur. Toutefois, la simplicité ne permet pas correctement traduire l’essai ni d’accéder à des paramètres d’usage courante en géotechnique (e.g. paramètres effort-déformation). Il s’avère nécessaire appliquer un modèle d’interaction plus réaliste, basé sur des paramètres permettant de caractériser le comportement du sol et adapté à cet essai.

Parmi les différents modèles existants, la modélisation numérique est, à l’heure actuelle, la méthode la plus réaliste pour reproduire le battage dans le sol. Elle sera un important outil dans ce travail afin mieux comprendre l’essai. D’une part, on souhaite vérifier si les hypothèses considérées dans la méthode d’obtention de la vitesse d’ondes sont valables et d’une autre part, le modèle numérique aidera à mettre au point le modèle d’interaction sol-pointe adapté. Un des atouts du modèle numérique est le fait permettre de confronter la réponse statique et dynamique sous des conditions identiques et ainsi d’aider à dissocier la réponde dynamique de la réponse statique. Une fois mis au point, le modèle d’interaction permettra de mieux saisir la composante dynamique dans la réponse globale de l’essai. On pourra ainsi estimer la réponse statique et l’exploiter à l’aide des modèles d’interprétation existants afin de remonter aux paramètres effort-déformation du sol.

\chapter{Introduction : \emph{rappel}}

La pénétration est utilisée par différentes techniques afin de caractériser le sol en surface. L'essai pénétrométrique statique consistent à pénétrer à une vitesse constante tandis que dans les essais dynamiques, cela se fait par le battage d'une masse. L'essai de pénétration dynamique est la technique \emph{in situ} la plus utilisée au monde. Toutefois, l'analyse des résultats dynamiques demeurent plus complexe que celle des essais statiques \citep{Byun2015CharacterizationPenetrometer}.

De manière générale, les pénétromètres, statiques ou dynamiques, fournissent une mesure à la rupture. Dans la pratique du dimensionnement géotechnique, celle-ci est appliquée directement ou interprétée afin d'estimer les paramètres de cisaillement (par exemple, l'angle de frottement effectif $\phi'$, pour les sables, la résistance non-drainé $\tau_u$, des argiles) \citep{Robertson1983InterpretationClay, Schnaid2008InTests, Mayne2001Stress-strain-strength-flowHistories}. Toutefois, les pénétromètres, \emph{a priori}, ne permettent pas d'établir la relation effort-déformation au cours des essais.

Dans l’état actuel des connaissances, l’essai de chargement dynamique en pointe pénétrométrique (DCLT, de l’anglais \emph{Dynamic Cone Load Test}) réalisé à l’aide du pénétromètre $Panda 3\up{\textregistered}$\footnote{Désormais noté Panda 3} \citep{Benz-Navarrete2009MESURES2, Escobar2015Mise3} permet de répondre à un grand nombre des besoins (coût, rapidité, haut rendement de mesures). L’instrumentation et l’exploitation des mesures de force et d’accélération au niveau de tête de l’appareil permettent d’estimer les grandeurs cinématiques à l’interface pointe-sol, à savoir : force, vitesse, déplacement.

La réponse globale du sol à l’aide de l’essai DCLT est exprimée sous la forme d’une courbe chargement en en fonction de l’enfoncement pour chaque impact (courbe charge enfoncement en pointe $\sigma_p-s_p$). Dans la méthode proposée par \cite{Benz-Navarrete2009MESURES2} et par \cite{Escobar2015Mise3}, l’exploitation de cette courbe s’appuie sur un modèle d’interaction pointe-sol de type Smith, très utilisé dans les essais de chargement dynamique de pieux. 

Il est représenté par un loi élastoplastique parfait avec une composante visqueuse de type newtonienne basé sur modèle masse-ressort-amortisseur de Smith (1962). Bien qu'intéressant, ce modèle ne permet pas caractériser le sol à l'aide de paramètres rhéologiques couramment appliqués en géotechnique.

L’objectif de ce travail est de mettre au point une méthode d’interprétation de l’essai de chargement dynamique en pointe pénétrométrique (DCLT). Cela afin de remonter aux propriétés intrinsèques du sol caractérisant son comportement (déformation et rupture) sous ce type de sollicitation. On s'intéresse aux paramètres utilisés pour le dimensionnement de ouvrages courants, à savoir le module de déformation, l'angle de frottement et la cohésion. Pour ce faire, on propose la démarche générale illustré par la schéma en Annexe (Figure \ref{fig:demarche}). 
...
Afin d’aboutir aux objectifs évoqués, la problématique est étudiée suivant trois approches, complémentaires les unes des autres, à savoir : analytique, expérimentale et numérique.
\begin{itemize}
 
\item Axe 1 : Approche Analytique :

L’étude bibliographique rassemble les enjeux plus importantes de ce travail de recherche ainsi que les tâches et verrous les plus lourds. Il constitue ainsi la base fondamentale de cette thèse. Il a pour but principal de guider le choix et l’adaptation des modèles d’interaction et d’interprétation appliqués à des problèmes comparables (battage des pieux, essais de pénétration). Ceci permettra de proposer un modèle bien adapté à la problématique (mode de sollicitation et déformations entraînées par l’essai).

\item Axe 2 : Approche expérimental :

Le développement de tout modèle explicatif ou prédictif nécessite des données issues de l’expérimentation. Cela afin d’évaluer et d’ajuster les prédictions faites par le modèle. Dans ce travail de recherche, la réalisation de quelques essais au laboratoire est envisagée afin de disposer des données expérimentales permettant d’adapter, d’ajuster et d’appliquer les modèles les plus pertinents. 

Toutefois, dans le cadre de ce projet de recherche, il ne sera pas possible d’analyser un nombre exhaustif de sols surtout à cause des limitations de temps. On dispose déjà de résultats expérimentaux sur différents sols et des essais complémentaires seront réalisés durant la thèse sur différents sols et des essais complémentaires seront réalisés durant la thèse par le compte de l’entreprise et par du personnel technique adapté permettant de calibrer et/ou valider les modèles développés. Cela devrait permettre de valider les modèles développés pour une gamme plus large de sols. A la fin et malgré tout, ces modèles nécessiteront toutefois d’être validés a posteriori pour une gamme plus large de sols, soit au laboratoire, soit in situ, dans le cadre de futurs travaux
...
Le présent rapport, qui reprenne une partie des principales activités réalisées tout au long de cette deuxième année de thèse, est divisé en trois partie, suivies d’un bilan général.
Dans un premier temps, on s'intéresse par les modèle d'interaction sol-pointe des pieux. Ceux-ci servirons pour améliorer la méthode d'interprétation de l'essai DCLT. 
Dans la deuxième partie de ce rapport, on présente brièvement les principaux essais réalisés au cours de l'année. Ceux-ci seront 
...
Bien que largement appliquée dans le domaine de pieu, la solution de Smith présente des grandes limitations. Certains travaux ont montré que l'amortissement radiatif (en anglais radiation damping) est fonction de la raideur du sol et non de la résistance. En plus, des résultats expérimentaux ont montré que la relation entre l'amortissement visqueux et la résistance statique n'est pas du type linéaire mais exponentielle \citep{Coyle1970EmpiricalClays}. Globalement, les principales limitations identifiées dans ce modèle sont : 

    • la non prise en compte de l’inertie du sol autour de la pointe ; 
    
    • les pertes d’énergie dû aux différents types d’amortissement (\emph{radiation damping}, l’hystérésis et à viscosité) ne sont pas considérées de manière isolée, mais combiné sur un seul paramètre ($J_s$) ;

    • la caractérisation du sol sur la base des paramètres d’usage peu courant en géotechnique (\emph{quake}, amortissement $J_s$).

Afin de rendre le modèle initialement proposé par \cite{Smith1960Pile-drivingEquation} plus réaliste, d'autres modèle ont été proposé par la suite. Ceux-ci cherchent à mettre au point des modèles alimentés par des paramètres ayant une signification physique et pouvant être déterminé en laboratoire.

\subsubsection{Modèle basé sur l'analogie de Lysmer}

Lysmer et Richard (1966) proposé une solution pour le déplacement d'une semelle circulaire rigide situé sur un massif semi-infini élastique parfait, homogène et isotrope soumis à un chargement vertical transitoire (type pulse). Dans cette solution, uniquement les efforts normaux sont transmis au niveau de l'interface sol-semelle.

Certains travaux se sont intéressé par la détermination des tassements d'une semelle, mais la solution de Lysmer a la particularité d'être valable pour toutes les fréquences de sollicitation, tandis que les solutions précédentes (Sung, Bycroft, Hsieh) s'appliquent à des faibles fréquences.

La réaction du sol sur la base de la semelle correspond à la somme de la réaction un ressort (fonction du déplacement) et de la réaction d'amortisseur (fonction de la vitesse). La raideur ($K_{Lys}$) de ce ressort celle de la solution statique (Équation \ref{eq:KLys}).

\begin{equation} \label{eq:KLys}
    K_{Lys} = \frac{4 G R}{1-\nu} 
\end{equation}

Soit G le module de cisaillement du sol, R le rayon de la semelle et $\nu$ le coefficient de poisson. Le coefficient d'amortissement est donnée par l'équation \ref{eq:CLys}.

\begin{equation}  \label{eq:CLys}
    C_{Lys} = \frac{3.4 R^2}{1-\nu} \rho  Vs = \frac{3.4 R^2}{1-\nu} \sqrt{\rho G}  
\end{equation}

Soit $\rho$ la masse volumique et $V_s$ la vitesse d'ondes de cisaillement du sol. L'amortissement dans le modèle de Lysmer représente l'amortissement radiation, c'est-à-dire dissipation d'énergie associée la propagation d'ondes (de volume et Rayleigh) dans le milieu semi-infini. Dans ce modèle, l'amortissement ne dépend pas de la fréquence. Le modèle basé sur l'analogie de Lysmer est adapté pour un sol élastique parfait soumis à un chargement vertical transitoire (pulse). La conformité la solution générale et les résultats pour un milieu semi-infini reste est suffisante pour les applications aux problèmes d'ingénierie.

La solution de Lysmer est largement employé dans le calcul de fondations de machine. ce modèle assume un comportement élastique parfait et le sol présente un comportement élastique restreint à des déformations très faibles. Par conséquent, le module G pour adapté pour déterminer la raideur K correspond au module à faibles déformation ($G_{max}$ ou $G_0$). Certains chercheurs suggèrent l'utilisation du module associé à la gamme de déformation du problème traité. La détermination la gamme de déformation associée au problème n'est pas simple (e.g. fonction de la distance de la source, du temps,...). Quant au module permettant de déterminer $C_{Lys}$, ceci est strictement le module à faibles déformations ($G_{max}$).
...
En effet, la solution présenté par Lysmer est valable pour le sollicitation statique et dynamique (toutes les fréquences), tant que l'encastrement soit nul (D=0) \cite{Loukidis2008AssessmentProcedure}. Pour les applications en profondeur, \cite{Loukidis2008AssessmentProcedure} propose l'intégration d'un facteur afin de tenir compte de l'encastrement. Ce sujet sera abordé plus tard dans ce rapport.

Récemment, \cite{Lee2020InstrumentedCharacterization} a appliqué l'amortissement matériel proposé par \cite{Lysmer1965VerticalFooting} afin de dissocier la parcelle dynamique de la résistance totale mesurée à l'essai au pénétromètre dynamique. Les auteurs ont montré qu'une fois la parcelle dynamique soustrait de la résistance totale, la résistance déduite était assez proche de la résistance pseudo-statique $q_c$ (CPT) (avec $q_c = 1.01 q_d$ et $R^2=0.85$). 

Plus tard, les travaux de Nguyen et al. (1988), de \cite{Holeyman1988ModellingBase} et de \cite{Deeks1995ALoading} ont permis d'améliorer la solution de \cite{Lysmer1965VerticalFooting}. Ils ont notamment avancer en prenant en compte les différents types d'amortissement (de radiation et d'hystérésis) que pour Smith étaient traduits pour un seul paramètre ($J_s$). En autre limitation de la solution de \cite{Lysmer1965VerticalFooting} est le fait de pas modéliser des milieux hétérogènes ou non-élastiques. Afin de corriger cette limitation, 

\subsubsection{Modèle de \cite{Loukidis2008AssessmentProcedure}}

\cite{Loukidis2008AssessmentProcedure} a proposé un modèle d'interaction qui tient compte de la non-linéarité de la réponse en pointe, de l'effet visqueux dans la résistance et qui distingue les différents types d'amortissement (radiation et hystérésis). La réponse totale en pointe R est la somme de la réaction du ressort $R_S$ et de l'amortisseur $R_D$ (Figure \ref{fig:ana_modele}).
...
Avec m et n des coefficients contrôlant l'effet de la vitesse. Le coefficient n présente une variation est assez petite quelle que soit le sol. Ceci est souvent pris 0.2 quel que soit le sol. Le coefficient m varie entre 0.34 et 0.56 pour les sables et entre 0.95 et 1.55 pour les argiles.

Lorsque la vitesse de nulle, $R_{Sf}$ est égal à $Q_{bL}$, soit $Q_{bL}$ la résistance limite en pointe. La résistance $Q_{bL}$ est égal au produit résistance unitaire et de la section du pieu. Selon \cite{Salgado2008TheFoundations}, la résistance unitaire en pointe est approximativement la résistance quasi-statique $q_c$ issue de l'essai CPT.

La raideur non-linéaire intègre les pertes d'énergie lié à la plastification (hystérésis du matériau). Le coefficient d'amortissement intègre l'amortissement radiatif (en anglais radiation damping) et la raideur tiennent compte de l'encastrement du pieu et de l'hystérésis du sol plus loin de la pointe réduit graduellement avec l'augmentation lorsque la réaction du ressort s'approche de la résistance en pointe ultime. 

La raideur maximale du ressort ($K_{max}$) et le coefficient de l'amortissement (C) sont basés sur les coefficient de Lysmer ($K_{Lys}$, Équation \ref{eq:KLys} et $C_{Lys}$, Équation \ref{eq:CLys}). Comme évoqué, la solution de Lysmer s'est basée sur une fondation chargée en surface. En réalité, dans le cas d'une fondation à une profondeur D, celle-ci va transmettre la chargement à un sol placé en profondeur et généralement plus rigide. Par conséquent, cet effet entraîne l'augmentation du coefficient $K_{max}$ \cite{Gazetas1985VerticalFoundations}. Il est ainsi important d'intégrer un facteur tenant compte de l'encastrement. \cite{Loukidis2008AssessmentProcedure} a analysé cet effet et a introduit le coefficient d'encastrement $D_f$. L'utilisation de ce coefficient permet d'appliquer la solution de Lysmer aux problèmes en profondeur.

\cite{Loukidis2008AssessmentProcedure} ont fait varié la profondeur d'une semelle au sein d'un matériau parfaitement élastique. Le coefficient $D_f$ est ainsi est estimé à partir du rapport entre la profondeur et la diamètre B (Équation \ref{eq:Df}). Ils ont observé également que sous chargement dynamique ce paramètre était élevé à 1,7.
...
Le module de déformation est globalement définie comme la relation entre la contrainte ($\sigma$) et la déformation ($\varepsilon$). Pour l'essai triaxial, la déformation est obtenue directement à partir de la hauteur initiale de l'échantillon et des déplacements mesurés au cours de l'essai. Dans le cas d'un essai de pénétration, son obtention n'est pas directe étant donné que les déformations non-homogènes.
...
Pour obtenir le module pénétrométrique, \cite{Faugeras1979LessaiAnalogique} introduit la notion de hauteur élastique équivalente ($h_{ee}$) afin de rendre $E_{pn}$ homogène à une contrainte. Cette hauteur correspond à la hauteur équivalente d'un échantillon cylindrique de 10cm2 de section, soumis à la même pression verticale, à une pression latérale égale à la pression des terres au repos à la profondeur de l'essai et qui aurait une déformation de $\Delta$h. Il doit tenir compte de la forme et des dimensions la plaque (représente dans ce cas par le cône), de l'encastrement mais aussi du remaniement du sol car, l'essai CLT est en fait un chargement de deuxième cyclique, le premier cyclique ayant été conduit jusqu'à la "rupture" du matériau.
...
La relation entre le module et la pente initiale appliqué par \cite{Arbaoui2003MesurePenetrometre} est la même proposé pour la raideur linéaire du ressort dans l'analogie de \cite{Lysmer1965VerticalFooting}. Cela est normal vu que la solution proposé par \cite{Lysmer1965VerticalFooting} est valable pour les sollicitation statiques et dynamique. En effet lorsqu'on écrit cette relation en fonction de la charge (Q) et du module de cisaillement (G) (Équation \ref{eq:G}), on obtient la raideur linéaire K du ressort du modèle de \cite{Lysmer1965VerticalFooting}.
...
Comme \cite{Loukidis2008AssessmentProcedure}, \cite{Arbaoui2003MesurePenetrometre} introduit un facteur d'encastrement (ici nommé $k_M$). Pour des grandes profondeurs (D>>B), la valeur de retenue est celle proposée par \cite{Mindlin1936ForceSolid} pour un encastrement infini ($k_M$=2). En égalant les expressions \ref{eq:E} et \ref{eq:h}, on obtient la hauteur $h_{ee}$ (Équation \ref{eq:hee}).
...
\cite{Zhou1997CaracterisationPanda} et plus tard \cite{Arbaoui2003MesurePenetrometre} ont réalisé une série d'essais CLT en laboratoire. Ils ont constaté que le module tangent de déchargement est sensiblement constant. Le module de chargement, celui-ci est pratiquement constant à partir du 3$^{ème}$ cycle. Le comportement global enregistré correspond à tout ce que l'on connaît des essais cycliques \cite{Zhou1997CaracterisationPanda}. Sur la Figure \ref{fig:ana_zhou_arbaoui_cycle}, on voit les résultats de \cite{Zhou1997CaracterisationPanda} et de \cite{Arbaoui2003MesurePenetrometre}. 
...
On sait que le terme de pointe (q) d'une semelle circulaire de diamètre B encastrée à une profondeur D peut être évaluée par la théorie de la capacité portante comme la somme de trois termes correspondant à contribution de la cohésion, du chargement et à l’effet du poids de sol (Équation \ref{eq:q(z)}).

\begin{equation}  \label{eq:q(z)}
    q(z) = s_c c N_c + s_q \sigma_v (z) N_q + \frac{1}{2} s_\gamma \gamma B N_\gamma
\end{equation}
...
$\sigma_v$ : contrainte verticale à la profondeur z ;

$\gamma$ : poids volumique du sol ; 

z : profondeur ; 

B : larguer de la fondation (ou diamètre du cône) ; 

$N_i$ : facteurs de la capacité portante sans dimension (i = c de cohésion, q de profondeur, $\gamma$ de surface) ; 

$s_i$ : coefficients de la forme (i = c, q, $\gamma$).

Pour le cas d'un forme circulaire, les coefficients de la forme s$_c$, s$_q$ et s$_\gamma$ sont, respectivement, 0.6, 0.3 et 1.0. Le troisième terme de l'équation (terme de surface) est négligeable pour les application en grandes profondeurs (D>>B). Or, dans le cas des sols purement frottant, le terme de cohésion est également négligeable (c$\approx$0) \citep{P.vandenBerg1996AnalysisPenetration}. Pour le cas de figure analysé : pointe de section circulaire de diamètre B égale à 22.5 mm à une profondeur D, où D>>B, au sein d'un sol purement frottant, l'équation de la capacité portante est réduite à l'équation \ref{eq:q(z)2}.

\begin{equation}  \label{eq:q(z)2}
    q(z) = \sigma_v (z) N_q 
\end{equation}

L'auteur a testé différentes solutions pour $N_q$ et il a finalement constaté que l'expression proposée par Caquot et Kérisel pour des fondations profondes, était la plus adaptée (Équation \ref{eq:q(z)3}).

\begin{equation}  \label{eq:q(z)3}
    N_q = 10^{N tg\phi}
\end{equation}

Avec le coefficient N variant de 3.7, pour les petits diamètres, à 2.7, pour les diamètres de 32 cm. Dans le cas de l'essai CLT, le diamètre étant assez petite, N est pris égal à 3.7. \cite{Arbaoui2003MesurePenetrometre} a confronté les valeurs de l'angle de frottement obtenu à partir de cette démarche ont été de l'ordre de 20\% plus important que les résultats triaxiaux. Il suggère que le coefficient N devrait être plus important, le diamètre du pénétromètre étant considérablement plus petit que celui des fondations profondes.
 
 \newpage
 
\subsection{Bilan et proposition pour l'essai DCLT}

On a pu voir l'effort des différents chercheurs afin d'améliorer le modèle de réaction initialement proposé par \cite{Smith1960Pile-drivingEquation}. Différents aspects du phénomène de la pénétration sont intégrés afin de mieux modélisé la résistance en pointe.

Les modèles développés plus tard montrent que la raideur du ressort et le coefficient d'amortissement ne sont pas proportionnel à la résistance limite, mais fonction du module de cisaillement du sol, de la densité du sol et rayon du pieu \citep{Lysmer1965VerticalFooting}, Nguyen et al. (1988), de \cite{Holeyman1988ModellingBase}, Deeks et Randolph (1995)).

Quasiment tout le modèle mis au point emploie l'analogie de Lysmer. Selon son analogie, la masse de frappe est placé dans la surface d'un massif semi-infini. Comme le rapport entre la profondeur (D) et le diamètre (B) est assez élevé pour le problème des pieux, la raideur et coefficient d'amortissement déduits devraient être considérablement plus petit que ceux pour le cas de pieux. \cite{Loukidis2008AssessmentProcedure} a étudié l'effet de l'encastrement. Ils proposent l'intégration d'un coefficient d'encastrement. Ceci est, à partir d'une profondeur importante (D>>B), uniquement fonction du coefficient de poisson du sol.

Malgré les particularités et les limitations de chaque modèle, on constaté certains points convergents :

- Pour les modèles intégrant l'effet visqueux dans la résistance, ceci est traduit par une relation proportionelle soit à la vitesse spot à la vitesse élevée par une coefficient. Par conséquent, pour ces différents modèles, au moment ou la vitesse en pointe est nulle, les effets visqueux devrait être négligeable.

- Quasiment tous les modèles de la réaction en pointe appliquent directement ou non l'analogie de Lysmer. Sur cela, \cite{Loukidis2008AssessmentProcedure} a présenté l'importance d'intégrer un coefficient d'encastrement afin d'adapter la raideur aux applications en profondeur. Cette raideur est reliée aux paramètres élastiques du sol, par conséquent, a priori déterminés à faibles déformations (\emph{e.g.} $v_s, G_{max}, v_p, E_{max}$).

\cite{Loukidis2008AssessmentProcedure} traduit la comportement non-linéaire du sol à partir de la dégradation de la raideur $K_{max}$ à partir d'un modèle hyperbolique.
...
Or, le pénétromètre est assimilable à un pieu isolé dont la réaction mesuré se doit à la réaction en pointe. Comme on évoqué, les méthodes développées dans le domaine de pieux permettent de relier la réaction en pointe aux paramètres rhéologiques (E, $\phi$, c, ...) du sol ausculté.

Dans les différents modèles évoques, on a vu que la réponse globale correspond à la somme de la réponse statique et dynamique. En plus, on a vu que, au contraire de ce que proposé \cite{Smith1960Pile-drivingEquation},  les différents amortissements (matériel, hystérésis et effet visqueux dans la résistance) sont expliquait par de mécanismes bien différents. L'amortissement matériel modélisé comme un amortisseur est fonction des paramètres élastiques du sol ($G_{max}$, $v_p$) \citep{Lysmer1965VerticalFooting, Loukidis2008AssessmentProcedure}.

L'amortissement d'hystérésis peut être traduit par la réduction de paramètres élastiques, par conséquent, de la pente de courbe. L'effet visqueux dans la résistance est proportionnelle à vitesse élevé par un coefficient. Cet effet doit être nul lors que la vitesse devient zéro.

%On sait que dans le cas de l'essai DCLT, les vitesses étant assez importantes, l'augmentation de la résistance attribuée aux effets visqueux sont non-négligeable. Toutefois, dans l'essai DCLT, on connaît les grandeurs (force, déplacement) tout au long du chargement, notamment lorsque la vitesse devient nulle. En ce moment précis, a priori, la résistance mesurée se doit uniquement à la réaction statique en pointe.
...
Selon la solution original proposé par Lysmer, cette raideur est fonction de paramètres élastiques du sol (\emph{i.e.} à faibles déformations). Le comportement élastique, initialement traduit par une raideur linéaire ne capte pas correctement la réaction hyperbolique observée expérimentalement. Pour \cite{Loukidis2008AssessmentProcedure}, cette raideur varie à partir de la valeur maximale (domaine élastique) avec l'enfoncement. Or, pour \cite{Loukidis2008AssessmentProcedure}, cette raideur maximale initiale correspondrait à la pente en décharge \citep{Holeyman1988ModellingBase}.

On sait que le sol présente un comportement élastique pour des déformations très petites (en général, $\varepsilon<10^{-4}$, \cite{Burland1989NinthStrains}). C'est pourquoi la plupart des techniques classiques ne permettent pas de remonter aux vrais paramètres élastiques ($G_{max}, E_{max}, v_s, v_p$). \emph{A priori}, l'obtention de ces paramètres requiert des méthodes bien spécifiques permettant d'entraîner des déformations suffisamment petites \citep{Burland1989NinthStrains, Atkinson2000Non-linearDesign}. 

\emph{In situ}, il est spécialement compliqué de déterminer les paramètres à faibles déformations. Souvent même l'installation de l'équipement ou le procédure avant le chargement entraîne un remaniement non-négligeable empêchant d'accéder à des paramètres associés à de déformation inférieure à 0.01. Étant donné cela, il serait compliqué de remonter aux paramètres élastiques à partir d'une courbe de chargement. 
...
Comme évoque par \cite{Loukidis2008AssessmentProcedure}, même si à la rigueur, la solution proposée \cite{Lysmer1965VerticalFooting} assume un comportement parfaitement élastique, dans la pratique la relation qui a proposé aussi appliquée pour de gamme de déformations plus importantes, notamment pour des problèmes dynamiques. Cette approche est celle appliquée dans le cadre d'autres travaux \citep{Zhou1997CaracterisationPanda, Arbaoui2003MesurePenetrometre, Reiffsteck2009MeasurementsPenetrometer} afin de remonter à un module pénétromètre.

Étant donné les constations évoquées, on propose d'exploiter la phase en décharge de la courbe DCLT afin de remonter à deux modules. En assumant que pour cette zone la raideur assume la valeur maximale ${K_{max}}$ et que la contribution des composant visqueuse est négligeable (le vitesse étant faible pour cette phase de l'essai).

On applique l'analogie de Lysmer afin de définir deux module de décharge. Comme suggère par \cite{Loukidis2008AssessmentProcedure}, on applique le coefficient d'encastrement dynamique (Équation \ref{eq:Df}). Les déformations associées à chacune de ces valeurs de module est définie à partir la définition du module et la déduction de la hauteur élastique équivalente (Équation \ref{eq:E} et Équation \ref{eq:hee}). Suivant cette démarche, on définie deux modules à partir de deux raideurs comme présenté sur la Figure \ref{fig:ana_Eun}.

  \begin{figure}[H]
   \begin{center}
        \includegraphics[scale = .7]{media/ana_Eun.png}
        \caption{Interprétation de la courbe CLT : détermination des modules $E_{un1}^{DCLT}$ et $E_{un2}^{DCLT}$}
          \label{fig:ana_Eun}
      \end{center}
 \end{figure}

Par la suite, on propose d'évaluer ses modules $E_{un1}^{DCLT}$ et $E_{un2}^{DCLT}$ à partir de résultats triaxiaux. Pour ce faire, on compare ces deux paramètres aux modules triaxiaux ($E_{0.01\%}$, $E_{50}$)pour les sables d'Hostun et de Fontainebleau à trois états de densités et trois confinements. Cela permettra de valider la pertinent de cette méthode pour les matériaux testés sur la base d'une méthode classique d'usage courant en géotechnique. 
...
Souvent, on n'a pas d'information sur la masse volumique \emph{in situ}. Dans la pratique, celle-ci est souvent assumé à 1800 $kg/m^3$ \citep{Burns2007Small-Penetrometer} ou encore estimée à partir les valeurs indicatives selon le type de matériau (sable, limon ou argile) \citep{Plumelle2013TheorieOuvrages}. Pour appliquer cette relation au sCPT, par exemple, ce paramètre est estimée à partir de la résistance de pointe ($q_c$). Ceci car la résistance de pointe est fortement corrélée à la masse volumique \citep{Burns2007Small-Penetrometer, Robertson2010EstimatingCPT}. 
...
Il s'avère nécessaire évaluer la pertinence de la courbe estimée. Pour ce faire, on propose des confronter les courbes obtenues à partir de cette démarche aux résultats rapportés dans la littérature. On envisage utiliser les résultats de \cite{Cami2017ImagerieDeformations} pour le limon du site expérimental de Jossigny et la courbe obtenue en laboratoire pour le sable d'Hostun sur le cylindre creux \citep{Duttine2005ComportementAxes}.
...
\chapter{EXPÉRIMENTAL}

Dans ce chapitre, on s'intéresse à une partie du travail expérimental réalisé en deuxième année. Les essais réalisés ont différents objectifs. Les essais laboratoire classiques (physiques et triaxiaux) servent d'une part à alimenter les lois de comportement adoptées dans le modèle numérique et d'une autre part à valider les paramètres rhéologiques déterminés par les essais DCLT.
...
Dans ce travail, on utilise le sable d'Hostun et de Fontainebleau. Ceux-ci sont des deux sables propres de granulométries proches, la principale différence entre ces matériaux étant l'angulosité des grains. On a soumis ces différents sols granulaires à des essais de caractérisation comprenant l'analyse granulométrie, la détermination des indices de vides maximale $e_{max}$ et minimale $e_{min}$ et la détermination de la densité relative des grains ($G_s$).
...
Le sable de Fontainebleau (SF ou, en anglais, FS) est un dépôt naturel issue de la région parisienne. Ce sable à granulométrie fine (<400$\mu$m) est essentiellement composé de silice (97 à 99 \%) ce qui lui confère la couleur blanche caractéristique. On utilise le matériau normalisé NE 34. Les grains ont une morphologie arrondie \citep{Aris2012ExperimentalElements}.
...
Quant aux résultats d'indices de vides ($e_{min}$ et $e_{max}$), on constate de variations. Concernant le protocole utilisé pour la détermination des indices des vides, on a suivi la norme française en vigueur (NF P 94-059). La valeur de $e_{min}$ pour le sable de Fontainebleau est légèrement supérieure à la moyenne des valeurs rapportées tandis que $e_{max}$ reste au-dessous. Pour ce sable, la variation entre les résultats et valeurs rapportées reste inférieure à 15\%. Pour le sable d'Hostun, $e_{min}$ est 22\% supérieure à la moyenne des valeurs rapportées tandis que $e_{max}$ varie très peu (-2\%) de la moyenne. 
...
Les facteurs clés influençant le comportement des sables propres sont l'état de contrainte et l'état de densité. Ainsi, les sables étudiés sont testés à différentes états de densité et pour différentes pressions de confinement. La Figure \ref{fig:exp_tx_eprouvette} synthétise les essais réalisés à l'appareil triaxial.

  \begin{figure}[H]
   \begin{center}
        \includegraphics[scale =1.]{media/exp_tx_eprouvette.PNG}
        \caption{Conditions des essais triaxiaux réalisés}
          \label{fig:exp_tx_eprouvette}
      \end{center}
 \end{figure}

\subsection{Conception de l'éprouvette et réalisation de l'essai}

Les éprouvettes reconstitués ont été préparées par compactage dynamique. Les sables ont été compacté par couche (de 3 à 6 couches) à l'intérieur du moule cylindrique (diamètre de 70 mm et hauteur de 140 mm) de façon d'éteindre la densité souhaitée. Différentes densités ont été réalisés allant de état lâche (DR$\approx$20\%) jusqu'à l'état très dense (DR$\approx$90\%). Pour chaque densité, on réalise trois éprouvettes à différentes pressions de confinement. L'essai est réalisé en conformité avec la norme française NF P 94-070 \citep{AFNOR1994NFRevolution}. Le protocole est celui d'un essai consolidé drainé avec mesure de pression interstitielle (CD+u).

Pour certaines éprouvettes, on modifie le protocole de la phase de cisaillement afin de réaliser un cycle décharge-recharge. Ceci afin de déterminer le module de décharge-recharge ($E_{ur}$) nécessaire pour alimenter la loi élastoplastique avec écrouissage (HSM) appliquée dans le modèle numérique \citep{Itasca2020FLAC3D6.0, Obrzud2018TheGuidebook}. La vitesse de déchargement et de rechargement est de celle du chargement. La vitesse de chargement est définie à partir de résultats de la phase de consolidation afin de garantir la condition drainée pendant toute la phase de cisaillement \citep{AFNOR1994NFRevolution}.
...
Du fait du comportement non-linéaire des sols, à partir de la courbe contrainte-déformation issue de l'essai triaxial, on peut définir différents modules. On s'intéresse principalement par modules sécants, c'est-à-dire, les modules calculés sur le rapport contrainte-déformation pour une déformation donnée de la courbe relié à l’origine, 0. Ces modules sont adapté principalement pour prédire le tassement d’un sol de fondation dû à l’application d’une charge (apportée par une semelle, par exemple) \citep{Briaud2001INTRODUCTIONMODULI}. 
...
Il est courant d'admettre que l'angle de frottement de pic des sols granulaires dépend de la densité et l'état de contrainte \citep{Rouse2018RelationTest,Lancelot1996ComportementDraines}. Toutefois, cette variation étant plus prononcé plus de contraintes faibles (< 50 kPa). Comme les essais triaxiaux présentés ont été réalisés à des pressions de confinement égales ou supérieures à 50 kPa, on assume que variation de $\phi'$, pour la gamme de confinement analyser, est négligeable. Ainsi, on présente les résultats en fonction de l'indice de vides (e). 

A titre de comparaison, on présente les résultats aux valeurs obtenues dans à l'appareil travail (CD) dans autres travaux pour ces sables. Pour sable de Fontainebleau \citep{Combarieu1997CapaciteLaboratoire, Dano2001ComportementInjectes, Dupla2007ComportementReference, Arbaoui2003MesurePenetrometre, Latini2016TriaxialReport} et pour le sable d'Hostun \citep{Lancelot1996ComportementDraines, Benahmed2004InitialSand}, Mokrani, 1991). En tenant compte de la variabilité associée le $\phi'$  issu l'essai triaxial, à savoir de 10\% \citep{Ameratunga2016CorrelationsEngineering}, on conclut que les résultats obtenus sont conformes à ceux rapportés dans la littérature pour ces sables.

Or, on constate que nos résultats, comme ceux rapportés, sont plus élevés pour le sable d'Hostun que pour le sable de Fontainebleau. Cela peut être expliqué en partie par l'angulosité des grains, le sable d'Hostun présentant des grains anguleux à sub-anguleux, tandis ceux de sable de Fontainebleau sont plutôt arrondis \citep{Aris2012ExperimentalElements, Benahmed2004InitialSand}.

  \begin{figure}[H]
   \begin{center}
        \includegraphics[scale=.7]{media/exp_frictionangle3.PNG}
...
Une série d'essais ont été réalisé en laboratoire utilisant les deux sables de référence (SF et SH) à différents états de densité. L'objectif de ces essais était, d'une part d'ajuster le modèle de comportement appliqué dans le modèle numérique et de comparer les deux techniques. 
...
Pour chacun de ces sables, deux échantillons ont été préparé par damage humide à différentes densités (voir \ref{fig:exp_eprouvettes}). Ces échantillons ont été préparés en huit couches de sol d'environ 10 cm de façon à garantir l'homogénéité de l'éprouvette. Le compactage dynamique a été réalisé manuellement à l'aide une dame. Des échantillons sont prélevés afin de déterminer la teneur en eau et d'obtenir la densité sèche en place. La Figure \ref{fig:exp_eprouvettes} montre les conditions des éprouvettes testés.

\begin{figure}[H]
\begin{center}
    \includegraphics[scale = .7]{media/exp_eprouvettes.png}
...
On poursuit avec l'essai CLT. La force et le déplacement sont enregistrés directement pendant que la vitesse de chargement est constante de 0,01 mm/s. On réalise de cinq à six cycles chargement-déchargement-rechargement.

Ensuite, on enlève les appareillages de l'essai CLT et on poursuit le chargement DCLT pour environ 10 cm. Cette procédure est répétée de façon à avoir réaliser des essais CLT à trois profondeurs différentes. Une fois le chargement CLT réalisé à la troisième profondeur, on poursuit avec l'essai DCLT jusqu'au fond du moule.
...
Dans les Figures \ref{fig:exp_DCLT-CLT_Hostun} et \ref{fig:exp_DCLT-CLT_Fontainebleau}, on constate que les deux techniques sont bien sensibles à la variation de la densité des éprouvettes. On observe que les séries de courbes DCLT sont assez répétables. Pour les courbes CLT, on observe des différences entre le première chargement et ceux qui suivent. Le premier chargement CLT présentant des contraintes légèrement plus faibles que les autres d'une même série. Cet effet est conforme aux constatation des \cite{Arbaoui2003MesurePenetrometre} et \cite{Zhou1997CaracterisationPanda}. Il peut être attribué en partie à la densification du sol autour de la pointe. Globalement, les courbes DCLT sont comparables au premier chargement CLT. 

En plus, on observe que les courbes CLT vertes des états plus dense présentent une augmentation des contraintes. Cet effet est encore plus expressif pour le sable de Fontainebleau (Figure \ref{fig:exp_DCLT-CLT_Fontainebleau}c). On attribue ceci à la proximité du fond de l'éprouvette.

\begin{figure}[H]
\begin{center}
    \includegraphics[scale = .7]{media/exp_DCLT-CLT_Hostun.png}
    \caption{Courbes CLT et DCLT pour le sable d'Hostun}
      \label{fig:exp_DCLT-CLT_Hostun}
  \end{center}
\end{figure}
...
\begin{center}
    ...
    \caption{Courbes CLT et DCLT pour le sable de Fontainebleau}
      \label{fig:exp_DCLT-CLT_Fontainebleau}
  \end{center}
\end{figure}

Pour ce cas de figure analysé, les courbes de chargement issues de deux techniques sont qualitativement comparables. Ces résultats permettront de plus tard d'ajuster la loi de comportement appliqué au modèle numérique.
...
L’heure actuelle, les méthodes numériques dites continus (\emph{e.g.} FEM, FDM) constituent l’approche la plus réaliste afin simuler l’interaction sol-pointe \citep{Loukidis2008AssessmentProcedure, Le2013NumericalApplications}. Celles-ci permettent de reproduire toute la complexité de la pénétration (comportement non-linéaire des sols, prise en compte des différents types d'amortissement, ...). C’est pourquoi, on propose  d'appliquer une méthode continue, et plus précisément en différences finies aux battage dynamique pénétrométrique. On souhaite développer un modèle numérique qualitativement et quantitativement représentatif de l'essai DCLT.

L'objectif de ce modèle est d’étudier l’influence des paramètres rhéologiques du sol ausculté dans le principal résultats de l'essai : la courbe charge-enfoncement dynamique. Les paramètres rhéologiques qu'on cherche sont les plus courants pour le dimensionnement en géotechnique : en déformation, le module d'élasticité (E) et, en cisaillement, l'angle de frottement interne ($\phi$) et la cohésion (c). En plus, ce modèle numérique permettra d’étudier: 

-	l’influence du type de sollicitation : statique et dynamique ;

-	les déformations entraînées par l’essai dans le sol.

Dans ce chapitre, on rappelle d'abord certains travaux réalisés dans le même sujet et certains aspects numériques importants pour comprendre le logiciel. Par la suite, on présente une partie du travail numérique réalisé au cours de la deuxième année. 
 
  \section{État de l'art}
  \label{sec:etat de l art}
On présente par la suite l'état de l'art sur la modélisation numérique. Dans un premier temps, on présente les modèles appliqués simulant l'essai DCLT et CLT et, par la suite, on s'intéresse aux modèles similaires à ceux qu'on souhaite modéliser : chargement dynamique à l'aide des méthodes continues.

  \subsection{Modélisation numérique de la pénétration dynamique}
  \label{subsec: modelisation numerique des essais DCLT et CLT}

La pénétration dynamique reste un de problème le plus sophistiqué et défiant des applications numériques en géotechnique. Il s’agit d’un problème fortement non-linéaire et qui entraîne les déformations très importantes aux maillages \citep{Nazem2012DynamicClay}.

Nombreux travaux numériques s’intéressent à simuler la pénétration dans les milieux granulaires. Parmi eux, la méthode discrète reste est la plus courante \citep{Zhang2019StandardChamber, Quezada2012MecanismesVariabilite, Arroyo2011ConeChamberb, J.Butlanska2010VirtualSand, Campos2005NUMERICALSOILS}. Les modèles rapportés qui s’intéressent à simuler la pénétration dynamique à l’aide des méthodes continues (FEM ou FDM) sont moins courantes \citep{Pinto2008EvaluationTesting, Nath1990AMETHOD, Aguiar2009NumericalProblem, Fakharian2014ComparisonModelling, Chatterjee2015DynamicCity, Murthy20203D-ContinuumMethod}. Parmi eux, les travaux simulant le battage sont encore plus rares. Généralement, la sollicitation dynamique est simulée en appliquant une force sinusoïdale à la tête du pieux \citep{Fakharian2014ComparisonModelling, Fakharian2015NumericalClay, Masouleh2008VerificationMethod, Pinto2008EvaluationTesting}.
...
\cite{Fakharian2014ComparisonModelling} ont simulé le battage de pieux à partir des deux approches continus (FEM et FDM). Les résultats de force, vitesse et déplacement observés dans le modèle sont conformes aux essais de chargement dynamique réalisés. Le but étant d’introduire l’utilisation des méthodes numériques continues afin de soulever certaines limitations du système masse-ressort-amortisseur (comme celle de \cite{Smith1960Pile-drivingEquation}). A la différence de la méthode masse-ressort-amortisseur, l’approche continue est alimenté par des paramètres mécaniques classiques (\emph{i.e.} E, c, $\phi$, $\nu$). Les paramètres du modèle unidimensionnelle de Smith, comme l’amortissement de Smith (J$_s$) et l’enfoncement élastique (quake) sont d’usage moins courant en géotechnique.
...
\section{Modélisation numérique de l'essai DCLT}
\label{subsec: DCLT}

Au cours de dernières décennies, de nombreux travaux se sont intéressés par la modélisation numérique de la pénétration à l'aide du pénétromètre Panda soit à partir de l’approche discrète \citep{Zhou1997CaracterisationPanda, Tran2019NumericalTest, Escobar2015Mise3, Tran2015ModelisationDiscrets, Benz-Navarrete2009MESURES2} soit par l’approche continue \citep{Zhou1997CaracterisationPanda, Arbaoui2003MesurePenetrometre}. Les objectifs de ces travaux étaient divers. 

\cite{Zhou1997CaracterisationPanda} et \cite{Arbaoui2003MesurePenetrometre} ont simulé l’essai CLT réalisé avec le pénétromètre Panda. Ils ont utilisé différents codes appliquant des méthodes continues (FLAC$^{2D}$, CESAR). Plus tard, \cite{Ali2010CaracterisationProfondes} simula l'essai CLT, cette fois-ci à l'aide avec le pénétromètre statique CPT. Leur but était, en autres, de réaliser l’analyse inverse de la courbe de déformabilité de cet essai et ainsi d’estimer des paramètres rhéologiques (E, c et $\phi$) du sol ausculté.
...
\cite{Zhou1997CaracterisationPanda} a aussi employé la méthode numérique discrète (PFC$^{2D}$) afin de valider le calcul de résistance de pointe Panda à partir de la formule des Hollandais ainsi que les hypothèses (choc élastique entre marteau et piston). L’approche discrète a été employée plus tard par \cite{Benz-Navarrete2009MESURES2} et par \cite{Quezada2012MecanismesVariabilite} afin d’étudier le mécanisme d’enfoncement. Les modèles discrets proposé par \cite{Tran2019NumericalTest} et par \cite{Tran2015ModelisationDiscrets} ont permis, entre autres, de valider la méthode d'obtention de la courbe charge-enfoncement en pointe à partir de l'exploitation des signaux mesurés au niveau de la tête.
 ...
 \section{Bref aperçu du logiciel : FLAC$^{3D}$}
 \label{sec:flac}
  
On emploie le logiciel $FLAC^{3D}$\footnote{\samepage Désormais nommé FLAC} (\emph{Fast Lagrangian Analysis of Continua in 3 Dimensions}). Il s’agit d’un logiciel aux différences finies explicite qui permet d’étudier le comportement mécanique d’un milieu continu en trois dimensions. Il applique des principes simples (définition des déformations et loi du mouvement) et des lois constitutives qui décrivent un matériau pour résoudre des équations différentielles à partir d’un schéma de discrétisation temporelle explicite. Les calculs sont réalisés en boucle pour chaque élément dans chaque pas de temps de façon indépendante. A la différence de la méthode aux éléments finis, la méthode explicite intégrée en FLAC ne requiert pas la création d’une matrice de rigidité. Par conséquent, ce logiciel peut suivre un chemin de contrainte-déformation non-linéaire sans une augmentation significative du temps de calcul (Figure \ref{fig:principe_flac}).

  \begin{figure}[H]
   \begin{center}
        \includegraphics[scale = .6]{media/num_flac.PNG}
        \caption{(a) Boucle de calcul explicite utilisé dans $FLAC^{3D}$ (modifié d'\cite{Itasca2020FLAC3D6.0}) et (b) Méthode lagrangienne versus méthode eulérienne \citep{Nagula2018EffectivenessModel}}
          \label{fig:principe_flac}
      \end{center}
 \end{figure}


La méthode consiste à trouver la solution d’un problème statique. Pour cela les équations du mouvement sont utilisées pour calculer des nouvelles vitesses puis des nouveaux déplacements à partir de contrainte/force en jeu. Ensuite, des taux de déformation en sont déduits à partir des vitesses et des lois de comportement du matériau sont appliquées afin de déterminer de nouvelles contraintes. Chaque parcours de cette boucle représente un cycle de calcul donc un pas de temps. Celui-ci doit être inférieur à une valeur critique afin de garantir la stabilité. Le pas de temps critique ($\Delta t_{crit}$) est donc gouvernée par la vitesse des perturbations se propageant dans le modèle et peut être estimée à partir de la taille minimale des éléments du maillage $z_{min}$ et de la vitesse des ondes les plus rapides (ondes primaires, $c_p$). Globalement, le pas de temps critique sera plus petit, et le calcul plus long, pour des maillages plus fins et pour des matériaux plus raides (Équation \ref{tempscritique}).

  \begin{equation} \label{tempscritique}
    \Delta t_{crit} < \frac{z_{min}}{c_p}
    \end{equation}
 
La limitation quant au pas de temps fait que cette méthode n’est pas très compétitive pour la résolution de problèmes linéaires en petites déformation où le pas de temps est petit. Elle s’avère intéressante pour les problèmes non-linéaires, des grandes déformations et des instabilités physiques.

Le critère de convergence du calcul est basé sur la force non équilibrée maximale agissantes  sur l’ensemble des noeuds du modèle. La force non équilibrée dans les noeuds correspond à la somme des forces extérieures appliquées au modèle et celles induites par la déformation des zones. Le rapport entre les forces nodales non équilibrées et les forces appliquées à tous ces noeuds permet d'évaluer la stabilité du modèle à chaque pas de temps. L'équilibre est atteint lorsque cette valeur devient très faible (inférieur à un seuil donné, par défaut de $10^{-5}$) ou sur un régime d’écoulement plastique permanent lorsque cette valeur est constante.

Dans FLAC, la méthode explicite utilisée est associée à une analyse lagrangienne. Les coordonnées des noeuds du maillage dans l’espace correspondent à des particules du milieu dont les positions sont référencées par rapport à la configuration initiale. Ainsi, la fonctionnalité dite « en grandes déformations » permet de modéliser des problèmes à grandes déformations car les coordonnées des nœuds sont mises à jour à chaque pas de temps. Cette méthode s’oppose à la méthode eulérienne où le matériau de déplace et se déforme tandis que le maillage est fixe.

Selon \cite{Ghee2011FLAC3DPiles}, un des avantages de la méthode intégrée dans FLAC constitue le qu’il fonctionne en petites ainsi qu’en grandes déformations. Dans la fonctionnalité dite en « grandes déformations », le maillage est mis à jour lors de déformations ou de mouvements. Ce mode est particulièrement intéressant pour traiter les problèmes à grandes déformations comme c’est le cas d’un essai de pénétration.

L’interface est modélisée comme une surface collée aux éléments afin de reproduire des déplacements relatifs et des impacts entre les différents éléments. Celle-ci est caractérisée par des paramètres de raideur et de rupture (critère de rupture Mohr-Coulomb). Ces paramètres doivent être choisis afin de minimiser son effet dans le modèle.


  \section{Mise au point du modèle numérique}
  \label{sec:developpement}
  
On décrit par la suite certains aspects de la mise en point du modèle, à savoir : géométrie, conditions aux limites, calibration du battage, modèle de comportement du sol et les paramètres rhéologiques.

  \subsection{Géométrie et conditions aux limites}
  \label{subsec:geo}
On souhaite simuler les essais DCLT et CLT au sein d’une chambre d'étalonnage cylindrique. Le pénétromètre est doté une tige de 14 mm de diamètre et d’une pointe conique débordante (sommet de 90\degree) de section transversale de 4 $cm^2$ (22,5 mm de diamètre). La géométrie de la chambre d'étalonnage ainsi que celle du pénétromètre sont présenté sur la Figure \ref{fig:exp_penetro}. Les conditions aux limites sont celles d’une chambre d'étalonnage type BC3 (déplacements empêchés latéralement et contrainte constante en surface). 

Dans les contextes du battage des pieux et des essais au pénétromètre dynamique, l’onde de contrainte engendrées par le battage se propage autour de la tige/pieu et se dissipe graduellement (dissipation d’énergie causée par la radiation des ondes au sein du matériau). Dans le but de reproduire cela et d’empêcher la possible la réflexion des ondes dans les parois, on introduit des parois dites « absorbantes ». 
...
La qualité du maillage est essentielle pour la précision et fiabilité des résultats. Un grand défi pour la modélisation continue de la pénétration est le fait d’entraîner grandes déformations entraînées dans le sol. Les limitations quant aux déformations du maillage varient selon la méthode intégré dans le code. En général, le code associant la méthode Lagrangienne et la méthode Eulérienne permettent de mieux gérer les déformations (\emph{e.g.} code Abaqus). La méthode Lagrangienne intégrée par le code FLAC permet de mettre à jour les coordonnées des noeuds au cours du calcul. Cette méthode n’est pourtant pas adaptée lorsque les déformations deviennent trop importantes \citep{Moug2019AxisymmetricClay, Nagula2018EffectivenessModel}.

C’est pourquoi la pénétration entraînée et le maillage adopté doivent être correctement choisi afin de limiter les distorsions et ainsi assurer la fiabilité de résultats. Lorsqu’on analyse les modèles utilisant le code FLAC pour la pénétration, on peut avoir une idée des ordres de grandeurs des pénétrations et des éléments du maillage.
...
Différents tailles d’élément de maillage ont été testé afin d’analyser la sensibilité des résultats. Le but étant de trouver un maillage suffisamment fin afin de converger vers une solution unique sans toutefois trop augmenter le temps de calcul. Le modèle est composé par 3048 éléments et 4664 noeuds.

Un autre aspect important concernant le maillage est la correcte propagation de l’onde de contrainte au sein de la tige. La modélisation numérique du phénomène peut produire des distorsions numériques dues à la taille des éléments (géométrie et maillage choisis). Afin de minimiser ce problème, \cite{Kuhlemeyer1973FiniteProblems} ont montré que les arêtes des éléments du maillage doivent être inférieure à 1/10 de la longueur de l’onde transmise. La longueur de l’onde est déterminée à partir de la célérité d’onde c et de la fréquence f (ou encore de la période T). 

L’onde de compression qu’on souhaite propager est caractérisée par une fréquence de l'ordre de 250 Hz et se propage dans la tige à une vitesse d'ondes de 5122 m/s. La taille minimale des éléments du maillage, de l'ordre de 2 m. Ce critère a été largement respecté avec l'arête plus important composant le maillage étant de l'ordre de 3 cm.


\subsection{Sollicitation}
\label{subsec:sollicitation}
  
On a simulé le chargement statique et le dynamique en pointe pénétrométrique. Le chargement statique a été simulé en appliquant une vitesse constante (0,01 m/s) à la face supérieure de l'extrémité de la tige. 

Comme évoqué, la plupart de travaux numériques rapportés dans la littérature simulent le chargement dynamique en appliquant une contrainte sinusoïdale à l’extrémité supérieure des pieux \citep{Fakharian2014ComparisonModelling, Masouleh2008VerificationMethod,Pinto2008EvaluationTesting}. Dans le cas de ce travail, ainsi que les travaux précédents \citep{Benz-Navarrete2009MESURES2, Escobar2015Mise3, Tran2019NumericalTest}, on a choisi de simuler le battage.

Pour ce faire, il s'avère nécessaire ajuster la sollicitation pour que celle-ci soit représentative de l'onde de choc observée expérimentalement. L'onde de compression générée suite à l'impact est triangulaire avec une légère asymétrie, celle-ci peut être caractérisée par la durée du choc ($\Delta t$) et une contrainte maximale ($\sigma_{max}$). On décrit par la suite la calibration du battage. Dans un premier, on vérifie la capacité de simuler la propagation d'une onde de compression.
...
Pour que la sollicitation dynamique simulée dans le modèle numérique soit véritablement représentatif de celle observée expérimentalement, on s'avère nécessaire assurer que l'onde d'impact soit comparable à l'onde expérimentale. On s'intéresse par la suite par la caractérisation de l'onde mesurée expérimentalement et l'ajustement de l'onde de compression numérique. Pour ce faire, une série d’essais ont été réalisée à l’aide d’un pendule en laboratoire. Le but est de caractériser l’onde engendrée par l'impact du marteau sous des conditions bien définies.

L'essai consiste à disposer le pénétromètre Panda 3 horizontalement reposé sur des roulements à billes et d'effectuer le battage d'aile d'un pendule de choc. Cela afin de maintenir son horizontalité au cours du battage et de s’affranchir les efforts de frottement le long des tiges. La disposition horizontale du système n’a aucune influence sur la propagation de l’onde car les forces gravitationnelles ne modifient pas la propagation de l’onde \citep{Fairhurst1959EnergyDrilling}.

L'utilisation du pendule à choc permet de maîtriser l’énergie de battage et, par conséquent, la vitesse de l'impact. Cet appareillage permet facilement faire varier l’énergie de battage et ainsi que la longueur du train de tiges. On emploie un train de tige assez long (à savoir de 7 m) afin d'empêcher la superposition des ondes incidentes et réfléchisses en sein du train de tiges. Comme conditions aux limites, la pointe à l'extrémité du train de tige fixé horizontalement. Le marteau est lâché en condition de pendule et heurte la tête du pénétromètre (voir Figure \ref{fig:num_exp2}).
...
L'énergie de battage ($E_b$) et la vitesse d'impact ($v_m$) du marteau sont exprimés respectivement par les équations \ref{eq:Eb} et \ref{eq:vm}.
...
Le marteau est composé de deux embouts en polystyrène de haut impact (HIPS) et une cavité cylindrique de parois de 2 mm en acier. celle-ci remplie de limaille de fer. Afin de simplifier l'analyse, dans le modèle, le marteau modélisé comme un cylindre homogène de 6 cm de diamètre et 13,6 cm de hauteur. Ceci est considère comme étant composé par un seul matériau, parfaitement élastique. Le module d'Young équivalent attribué au marteau $(E_m)$ est déduit à partir de sa géométrie et des modules de l'acier et du HIPS. On a adopté des valeurs de module pour la HIPS de 0,5 GPa, selon les informations du fabricant (Tableau \ref{tab:marteau}).
...
Pour simuler l'impact entre deux objets dans FLAC, on doit créer une interface. Comme déjà expliqué dans ce rapport, l'interface est accolée à une (ou les deux) faces qui interagissent. Son comportement est caractérisé par des paramètres de rupture ($\phi$ et c) et de déformation (raideur normale $k_n$ et tangentielle $k_s$). 

Une fois le module du marteau défini, on s'intéresse à ajuster la raideur normale de l'interface afin d'avoir la durée de choc observé expérimentalement ($\approx$ 1,5 ms). La Figure \ref{fig:num_raideur} présente les signaux de force mesuré dans tige pour les différentes valeurs de raideur $k_n$ testés. On présente ainsi que le signal expérimentale moyen pour la même vitesse d'impact (6,25 m/s). On constate qu'une raideur de $2 . 10^{10}$ Pa/N permet d'engendrer une onde représentative de celle expérimentale en terme de durée de choc ($\approx$ 1,25 ms) et force maximale $F_{max}$ (Figure \ref{fig:num_raideur}). 
  \begin{figure}[H]
   \begin{center}
...
En effet, les différents sources de pertes énergétiques présentes dans l'essai ne sont par intégrés dans le modèle numérique ceci étant non-amorti. Afin de corriger cela, on introduit un coefficient a. Ceci sera appliqué à la vitesse d'impact expérimentale afin de déduire la vitesse à être utilisé dans le modèle numérique.
...
\subsection{Modélisation de l'interface sol-pointe}
\label{sec:interface}

Afin de simuler l’interaction entre le pénétromètre et le sol, il s’avère nécessaire introduire une interface entre les deux matériaux. Celle-ci permettra de correctement reproduire le choc au niveau de la pointe et le glissement entre les parois de la pointe et le sol. Dans FLAC, l’interface est modélisée par un maillage de triangles définis par trois noeuds placés sur la surface de l’objet. On a crée deux interfaces au niveau de la pointe : une latérale et une au sommet (Figure \ref{fig:num_interface2}). 

Le comportement de l’interface est caractérisée par (a) les paramètres de raideur : normale k\textsubscript{n} et tangentielle k\textsubscript{s} données en contrainte par mètre (Pa/m), et (b) de rupture suivant le critère Mohr-Coulomb, avec la cohésion (en Pa) et les angles de frottement et de dilatance (en dégrée). 
...
\begin{minipage}{0.45\textwidth}
   \begin{figure}[H]
    ...
    \caption{Définition de $\Delta Z_{min}$ pour le calcul de $k_n$}
    \end{figure}
\end{minipage}%
%\hfill
\begin{minipage}{0.45\textwidth}
...
On conclut que l'équation \ref{eq:ks} permet d'estimer à partir de paramètres de déformabilité du sol, les raideurs d'interface minimales afin de garantir que celle-ci n'intervienne pas significativement dans les résultats.
...
En assumant que variation de $\phi'$ avec le confinement est négligeable, on a tracé l'évolution de $\phi$ en fonction de l'état de densité de trois éprouvettes testées à l'appareil triaxial. Le but étant d'estimer l'angle de frottement correspondant à l'état de densité de des éprouvettes réalisées (e = 0,68 et e = 0,78, voir Figure \ref{fig:exp_eprouvettes}). La relation entre l'indices de vides et l'angle de frottement est présenté dans la Figure \ref{fig:exp_phi_SH_SF_tx}. Les valeurs de $\phi'$ obtenues sont : 30,7\textdegree{} et 34,5\textdegree{} pour les éprouvettes lâches et moyennement dense, respectivement.

L’angle de dilatance $\psi$ est quant à lui généralement compris entre 0\degree et 15\degree. Les sables lâches et les argiles ont des valeurs de dilatance très faibles, quelques degrés à peine voire zéro. Il est souvent déterminé en laboratoire à partir de résultats de l'essai triaxial. Il existe également des relations empiriques simples reliant l’angle de dilatance et l’angle de frottement interne \citep{Levasseur2007AnalyseGenetiques}. De manière similaire à \cite{Ahmadi2005Thin-layerMeasurement}, on estime l'angle de dilatance à partir de l'angle de frottement en rupture et l'angle de frottement critique (Équation \ref{eq:dilatance}). Comme on a évoqué, les valeurs rapportées dans la littérature pour le $\phi_{cr}$ varient peu. Ceux-ci sont compris entre 29,3\textdegree{} et 31,0\textdegree{} \citep{Dano2001ComportementInjectes, Dupla2007ComportementReference, Vernay2017EtudeLiquefaction, Andria-Ntoanina2011CaracterisationCentrifugeuse}.
...
Dans ce rapport, après un rappel du contexte, objectifs, on présente une partie du travail réalisé au cours de la deuxième année de la thèse. Ces travaux serviront plus tard à améliorer l'interprétation de l'essai DCLT et à valider les paramètres obtenus. 

Dans l'année qui suit, il faudra appliquer un modèle d'interaction sol-pointe plus réaliste (\cite{Loukidis2008AssessmentProcedure} afin de dissocier la composante dynamique et de pouvoir appliquer des modèles d'interprétation comme l'expansion de cavité à cet technique.

Or, les améliorations réalisés dans le modèle numérique serviront atteindre un des objectifs de cet travail : de simuler l'essai DCLT et aussi d'aider à appliquer le modèle d'interprétation sol-pointe.
...
\begin{sidewaysfigure}[ht]
    ...
    \begin{center}
    \includegraphics[scale = 1.1]{media/ana_demarche.PNG}
