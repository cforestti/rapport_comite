...
En assimilant l'essai DCLT à un essai de chargement de pieu en échelle réduite, on souhaite appliquer les modèles d'interaction proposés pour les pieux. A l'exception de modèle de Smith, tous les modèles présentés se sont basés sur le coefficient de Lysmer ($K_{b,Lys}$) exprimant le déplacement en pointe en fonction des paramètres de déformabilité du sol ($\nu$, $G$). Pour les utilisation en profondeur, \cite{Loukidis2008} ont proposé de corriger l'effet d'augmentation de $K_{b,Lys}$ à l'aide des coefficients d'encastrement ($D_f$ ou $D_{f,dyn}$).
