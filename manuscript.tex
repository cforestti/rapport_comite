...
%\hskip2mm
\begin{spacing}{1.}
La caractérisation de sols est une condition indispensable pour le dimensionnement des fondations en génie civil. A niveau mondial, les travaux de caractérisation, dont l’objectif est de fournir les paramètres du sol permettant d’adapter l’ouvrage au terrain, sont basés sur la réalisation d’essais in situ. Au-delà des limites techniques, la plupart de ces méthodes ne permettent pas d’avoir une caractérisation fine des sols présents en surface à faible coût. Par ailleurs, à l’heure actuelle, la plupart des essais géotechniques in situ ne permettent pas d’identifier directement les paramètres constitutifs des couches de sol. Un pénétromètre dynamique léger instrumenté (Panda$3\up{\textregistered}$) a été développé afin de répondre à ces besoins.

L’instrumentation et l’exploitation des signaux de force et d’accélération intégrés à cet appareil permettent d’obtenir une courbe de chargement en pointe du sol pour chaque impact. Celle-ci traduit la réponse effort-enfoncement sous chargement dynamique et riche de renseignements quant au comportement en déformation et en cisaillement des sols. Le caractère dynamique et transitoire de l’essai couplé à la précision et grand nombre de mesures obtenues facilite la mise en place de différentes méthodes d’analyse intégrant différentes approches (essais de choc, analyse temporelle, analyse spectrale, fonction des transfert …). 
...
La plupart des essais géotechniques in situ ne permettent pas d’identifier directement les paramètres constitutifs des sols. Les essais de laboratoires quant à eux coûtent chers et ne sont pas représentatifs du massif de sol dans son ensemble
...
Afin de rendre le modèle initialement proposés par \cite{Smith1960} plus réaliste, d'autres modèle ont été proposé par la suite. Ceux-ci cherchent à mettre au point des modèles alimentés par des paramètres ayant une signification physique et pouvant être déterminés en laboratoire \citep{Holeyman1988}.
...
\subsection{Intégration de la non-linéarité}
...
A l’heure actuelle, les méthodes numériques dites continues (\emph{e.g.} FEM, FDM) constituent l’approche la plus réaliste afin de simuler l’interaction sol-pointe \citep{Loukidis2008, Le2013}. Celles-ci permettent de reproduire toute la complexité de la pénétration (comportement non-linéaire des sols, prise en compte des différents types d'amortissement, ...). Par ailleurs, la simulation numérique a plusieurs intérêts :
\begin{itemize} [label=\textbullet]
\item on peut agir facilement sur les paramètres et étudier ainsi leurs influences sur le comportement du système : par exemple, on peut faire varier des paramètres rhéologiques (E, phi, ...), la géométrie des éléments (marteau, pointe, ...) ainsi que les conditions aux limites ;
\item on connaît à tout moment l'état du système. Cela veut dire qu'on peut faire des mesures qui ne perturbent pas le comportement du système. Par exemple, suivre les grandeurs cinématiques à des emplacements où il serait difficilement praticable expérimentalement notamment à l'interface sol-pointe ; 
\item on peut faire un grand nombre d'expériences à peu de frais. Cela est spécialement intéressant dans le cas de la méthode continue celle-ci étant moins couteuse en terme de temps que la méthode discrète ;
\item on peut refaire plusieurs fois une expérience avec strictement les mêmes conditions initiales.
\end{itemize}

C’est pourquoi, on propose d'appliquer une méthode continue à l'essai DCLT. On a développé un modèle numérique qualitativement et quantitativement représentatif de cette technique. L'objectif de ce modèle est d’étudier l’influence des paramètres rhéologiques du sol ausculté sur le principal résultat de l'essai : la courbe charge-enfoncement dynamique (ou courbe DCLT). Les paramètres rhéologiques étudiés sont les plus courants pour le dimensionnement en géotechnique : en déformation, le module d'élasticité (E) et, en cisaillement, l'angle de frottement interne ($\phi$) et la cohésion (c). En plus, ce modèle numérique permettra d’étudier : 
\begin{itemize} [label=\textbullet]
\item l’influence du type de sollicitation : statique et dynamique ;
\item les déformations entraînées par l’essai dans le sol.
\end{itemize}

Dans ce chapitre, on rappelle d'abord certains travaux réalisés autour de la modélisation numérique de la pénétration dynamique et certains aspects numériques importants pour comprendre la méthode appliquée. Par la suite, on présente les différents étapes et aspects de la mise au pointe du modèle.
 
  \section{État de l'art}
  \label{sec:etat de l art}
  
On présente par la suite l'état de l'art sur la modélisation numérique. Dans notre étude, initialement, on va se focaliser sur les modèles numériques appliquant des méthodes continus à la pénétration dynamique (battage de pieux ou pénétromètre dynamique). Additionnellement, on abordera les modèles numériques développés autour des essais DCLT et CLT.
...
De nombreux travaux numériques s’intéressent à simuler la pénétration dans les milieux granulaires. Parmi eux, un nombre de travail ont simulation des essais de pénétration dynamique (SPT et DCP) à l'aide de la méthode discrète \citep{Zhang2021c,Zhang2019, Quezada2014a}. D'autres travaux rapportés simulent la pénétration dynamique à l’aide des méthodes continues (FEM et FDM) \citep{Murthy2020, Chatterjee2015a, Fakharian2014,  Aguiar2009, Pinto2008, Nath1990}. Parmi eux, les travaux simulant le battage sont assez rares. Généralement, la sollicitation dynamique est simulée en appliquant une force sinusoïdale à la tête du pieux/pénétromètre.
...
%La méthode des différences finies consiste à partir d’un milieu continu, à écrire les équations qui régissent ces milieux en imposant les lois de conservation et de comportement, puis à discrétiser les équations obtenues en transformant un système d’équations différentielles en équations algébriques. 

%La méthode des éléments finis discrétise le milieu continu, en lui attribuant un nombre fini de degré de liberté et c’est seulement dans un second temps qu’elle impose, en moyenne, les lois de conservation et de comportement. Les équations algébriques qui régissent le problème émergent ainsi suite à la discrétisation initiale.
...
Un état initial de contraintes d’origine géostatique est pris en considération. Il correspond à l’effet du poids de couche de sol, avec le poids volumique sec $\gamma_d$. Dans le modèle, la contrainte latérale est engendrée par le coefficient des terres au repos $K_0$, $K_0$ étant le rapport entre les contraintes radiales et verticales ($K_0 =\sigma_{rr}/\sigma_{zz}$). Pour les sables normalement consolidé, ceci est estimée à l’aide de la relation proposée par \cite{Jaky1944} : $K_0=1-\sin \phi'$ , avec $\phi'$ l’angle de frottement drainé du massif. Le choix de l'angle de frottement ainsi que d'autres paramètres physiques et rhéologiques sera aborder plus tard dans ce rapport. Une cavité est crée au sein du massif afin de donner lieu au pénétromètre. Les parois de cette cavité sont stabilisés, c'est-à-dire les déplacements sont empêchés latéralement
...
Premièrement, le comportement du contact est décrit par une interface " rigide " où seulement le glissement et le décollement peuvent se produire, et la déformation de l’interface n’est pas prise en compte. Dans ce cas, les paramètres définissant le critère de rupture sont importants alors que les valeurs des rigidités le sont moins.  Néanmoins, les contrastes de rigidités importants sont très mal tolérés sous FLAC. Les valeurs des rigidités très élevées conduisent à une convergence de solution extrêmement lente et les temps de calculs peuvent alors être trop longs et prohibitifs. Il est donc essentiel de déterminer les rigidités avec justesse.

Afin de simuler le glissement et le décollement entre le pénétromètre et le sol, il s’avère nécessaire introduire une interface entre ces deux matériaux. Celle-ci permettra de reproduire correctement le choc au niveau de la pointe et le glissement entre les parois de la pointe et le sol. Dans FLAC, l’interface est modélisée par un maillage de triangles définis par trois nœuds placés sur la surface de l’objet, ici la pointe du pénétromètre. Celle-ci est caractérisée par des paramètres de raideur et de rupture (critère de rupture Mohr-Coulomb). Ces paramètres doivent être choisis afin de minimiser son effet dans le modèle.
...
Le comportement du contact sol-pointe est décrit par une interface rigide, c'est-à-dire, seulement le glissement et le décollement peuvent se produire, et la déformation de l’interface est négligeable. Pour cela, il faut choisir des valeurs de rigidités suffisamment grandes afin que la déformation élastique qu’elle engendre soit négligeable. Des rigidités très élevées conduisent à des pas de temps importants ainsi à des temps de calculs trop longs. Cela étant donné, il est pertinent de choisir les rigidités de l'interface avec justesse.

Les raideurs apparentes de l’interface d’une zone perpendiculaire sont souvent estimées à partir de la taille de l’élément le plus petit dans la direction normale $\Delta Z_{min}$ et du module (E) du milieu adjacent \citep{Itasca2020}. \emph{A priori}, les raideurs de l’interface sont estimées à partir du module d'élasticité du matériau le plus raide, à l’exception des problèmes où les modules des deux matériaux sont trop différents. Dans ce cas spécifique, on estime les raideurs de l’interface à partir du module du matériau le moins raide (ici, le sol) (Équation \ref{eq:ks}).
...
La réponse mécanique se compose d’une première phase de montée en charge rapide correspondant à l’augmentation initiale de la vitesse de la tige. Dans un second temps, l’effort sur la tige reste positif pendant l’enfoncement de la tige jusqu’à sa pénétration maximale. Enfin, on observe une phase de déchargement-rechargement correspondant à la stabilisation de la tige autour d’une position d’équilibre finale.
 ...
 \begin{figure}[H]
   \begin{center}
        \includegraphics[scale = .5]{media/num_DCLT_in.png}
...
On simule les essais de chargement en pointe dynamique et statique au sein des massifs représentation le sol.

A ce massif, on applique deux modèle de comportement : élasto-plastique parfait Mohr-Coulomb et le modèle élastoplastique avec écrouissage (Hardening Soil Model ou HSM). Le modèle HSM permet de mieux tenir compte des déformations irréversibles qu’on observe dans le sol même loin de la rupture. Ce concept est dérivé du comportement des métaux écrouis (hardened) dont le niveau de plasticité augmente avec l’intensité des déformations plastiques subies. 

Les paramètres alimentant les modèles de comportement sont ceux obtenus à l'appareil triaxial pour des échantillons des sables étudies (SF et SH).

En effet, on observe que les modules d'élasticité nécessaires afin d'avoir une correspondance avec les résultats expérimentaux doivent être considérablement plus importants que ceux obtenu à l'état de densité et une contrainte moyenne à l'essai triaxial. En effet, le rapport entre les modules nécessaires et ceux qu'on attribue au matériau est de l'ordre 6. 

Cette différence a été observé par d'autres travaux comparant des courbes numériques et expérimentales de chargement de pieu. Ces travaux ont observé des rapports du même ordre \citep{Pinto2008, Masouleh2008, Fakharian2014}. En effet, l'essai de pénétration ainsi comme dans l'installation d'un pieu entraine une densification dans le sol. En pointe, cet effet est similaire à un pré-chargement dans la zone adjacente à la pointe. Toutefois, cet effet est très de difficile de quantifier. \citep{Lee1999}. Comme dans tous les travaux rapportés dans la littérature, des comparatifs directes entre les résultats expérimentaux et numériques n'est pas praticable.   
...
Les études expérimentales et analytiques proposés ont été complétées par la modélisation numérique en différences finies (logiciel FLAC3D). Ce modèle numérique étant capable de simuler les essais de chargement statiques et dynamique en pointe. La géométrie et les conditions aux limites sont celles des essais expérimentaux en chambre d'étalonnage. Pour simuler la sollicitation dynamique, au lieu d'introduire un signal de force comme dans le cas d'autres travaux, on a simulé le battage et la propagation de l'onde. Cela permet d'être plus réaliste par rapport à la technique et de mieux se caler par rapport aux résultats expérimentaux.

L'étude bibliographique réalisée à propos des modèles d'interaction a permis de souligner leur points convergeant et leurs limitations. A l'issue de cette étude, on envisage d'appliquer un modèle d'interaction sol-pointe alimenté par des paramètres caractérisant le sol d'application courante en géotechnique.


% afin de dissocier la composante dynamique et de pouvoir appliquer des modèles d'interprétation comme l'expansion de cavité à cette technique. 
