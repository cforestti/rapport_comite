...
C’est pourquoi, on propose d'appliquer une méthode continue aux battage dynamique pénétrométrique. On a développé un modèle numérique qualitativement et quantitativement représentatif de l'essai DCLT. L'objectif de ce modèle est d’étudier l’influence des paramètres rhéologiques du sol ausculté sur le principal résultat de l'essai : la courbe charge-enfoncement dynamique (ou courbe DCLT). Les paramètres rhéologiques étudiés sont les plus courants pour le dimensionnement en géotechnique : en déformation, le module d'élasticité (E) et, en cisaillement, l'angle de frottement interne ($\phi$) et la cohésion (c). En plus, ce modèle numérique permettra d’étudier : 
\begin{itemize} [label=\textbullet]
\item l’influence du type de sollicitation : statique et dynamique ;
\item les déformations entraînées par l’essai dans le sol.
\end{itemize}

Dans ce chapitre, on rappelle d'abord certains travaux réalisés sur le même sujet et certains aspects numériques importants pour comprendre le logiciel. Par la suite, on présente les différents aspects du modèle numérique.
