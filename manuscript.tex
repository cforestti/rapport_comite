...
\subsection{Vérification numérique : application du coefficient de Lysmer et l'encastrement à l'essai DCLT}
 
En assimilant l'essai DCLT à un essai de chargement de pieu en échelle réduite, on souhaite appliquer les modèles d'interaction proposé pour les pieux à cet essai. A l'exception de modèle de Smith, tous les modèle présenté se sont basé sur le coefficient de Lysmer ($K_b$) d'exprimer le déplacement en pointe en fonction des paramètres de déformabilité du sol ($\nu$, $G$). Pour les utilisation en profondeur, certains travaux proposé de corriger l'effet d'augmentation de ($K_b$) à l'aide d'un coefficient ($D_f$).
...
Il s'avère importante vérifier si ces relations sont valables pour l'essai DCLT. Cela pour les conditions comme celles supposé par \cite{Lysmer1965} et par \cite{Loukidis2019} : milieu parfaitement élastique, isotropique et homogène. On souhaite vérifier si pour un milieu comme celui supposé par \cite{Lysmer1965}, en simulant l'essai DCLT, il est possible de remonter au module d'élastique en appliquant le coefficient de Lysmer. A la différence de la solution initial proposé par l'auteur, ici on respectera la géométrie du pénétromètre. Cela permettra de vérifier de sa solution, développé pour une plaque cylindrique chargé, est valable pour la géométrie de l'essai DCLT : pointe débordante. Par ailleurs, on vérifie si le coefficient proposé par \cite{Loukidis2008} pour également pour une plaque encastrée est valable pour l'essai DCLT.

Afin de vérifier cela, on a réalisé 
...
Or, le caractère dynamique transitoire du chargement rend plus complexe la définition de cette valeur de résistance. Cela car la composante dynamique présente dans le résultat obtenue n'est pas négligeable et est fonction, entres autres, de la vitesse de pénétration.

Dans le cas d'un pieu isolé sous chargement statique, la résistance ultime total $Q_{ult}$ est la somme des résistances latérales et en pointe.
