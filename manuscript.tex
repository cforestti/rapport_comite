...
Différents modèles analytiques de la réaction en pointe essaient d'exprimer la réponse observée pour les essais de chargement statique de pieux en reliant à contrainte (ou la charge) à l'enfoncement, à savoir : \cite{Smith1960}, \cite{Lysmer1965}, Nguyen et al. (1988), \cite{Holeyman1988}, \cite{Naggar1994} et celui proposé plus récemment par \cite{Loukidis2008}.
...
les fondations profondes D>0. Pour les fondations encastrées, le coefficient K observé est supérieur à la valeur théorique.
...
La courbure de l'hyperbole dépend du type de pieux et du sol. On assimile le pénétromètre à un pieu battu. \cite{Loukidis2008AssessmentProcedure} observe que $b_f$ varie entre 1,0 et 2,0 pour les pieux battus. L'effet visqueux dans la résistance sont pris en compte d'après la relation proposé par \cite{Coyle1970} (Équation \ref{eq:Coyle}).
...
\cite{Loukidis2008} ont fait varié la profondeur d'une semelle au sein d'un matériau parfaitement élastique. Le coefficient $D_f$ est ainsi estimé à partir du rapport entre la profondeur et le diamètre B (Équation \ref{eq:Df}). Ils ont observé également que sous chargement dynamique ce paramètre était élevé à 1,7 (Equation \ref{eq:Df_dyn}). Pour les rapports D/B>8, $D_f$ devient constant, $D_f$ varie entre 1,36, pour $\nu$=0,5 (argiles saturées), et 1,5, pour $\nu$=0,15 (sables).
...
La relation entre le module et la pente initiale appliquée par \cite{Arbaoui2003} est la même que celle proposée pour la raideur linéaire du ressort dans l'analogie de \cite{Lysmer1965VerticalFooting}. Cela est normal vu que la solution proposée par \cite{Lysmer1965VerticalFooting} est valable pour les sollicitations statiques et dynamiques. En effet lorsqu'on écrit cette relation en fonction de la charge (Q) et du module de cisaillement (G) (Équation \ref{eq:G}), on obtient la raideur linéaire K du ressort du modèle de \cite{Lysmer1965VerticalFooting}.
...
Comme \cite{Loukidis2008}, \cite{Arbaoui2003} introduit un facteur d'encastrement (ici nommé $k_M$). Pour des grandes profondeurs (D>>B), la valeur retenue est celle proposée par \cite{Mindlin1936ForceSolid} pour un encastrement infini ($k_M$=2). En égalant les expressions \ref{eq:E} et \ref{eq:h}, on obtient la hauteur $h_{ee}$ (Équation \ref{eq:hee}).
...
\cite{Arbaoui2003MesurePenetrometre} évoque l’utilité de la réalisation des cycles chargement-déchargement-rechargement au lieu d'un simple chargement monotone. On peut évoquer, à l’appui d’un tel essai, l’espoir d’effacer, en partie, les effets du remaniement \citep{Combarieu2001}. 
...
\cite{Zhou1997} et plus tard \cite{Arbaoui2003} ont réalisé une série d'essais CLT en laboratoire. Ils ont constaté que le module tangent de déchargement est sensiblement constant. Le module de chargement est, quant à lui, pratiquement constant à partir du 3$^{ème}$ cycle. Le comportement global enregistré correspond à tout ce que l'on connaît des essais cycliques \cite{Zhou1997CaracterisationPanda}. Sur la Figure \ref{fig:ana_zhou_arbaoui_cycle}, on voit les résultats de \cite{Zhou1997} et de \cite{Arbaoui2003}. 
...
Afin de vérifier dans premier temps la faisabilité, dans un premier temps, on a simulé un essai similaire, mais plus simple : le SPT. En effet, le battage SPT étant plus simple (en terme de géométrie et de matériaux utilisés), le problème a été déjà modélisé analytiquement \citep{Schmertmann1979, Gonin1979, Fairhurst1959}.

Ces travaux ont constaté des bonnes correspondances entre les prédictions analytiques à des résultats expérimentaux. En suivant la même démarche, on souhaite vérifier si on est capable de simuler le battage SPT en différences finies. Par la suite, on s'intéressa la transmission d'énergie entre le marteau et la tige. La compréhension des phénomènes dynamiques ayant lieu après l'impact permet d'estimer les forces.
...
Il est possible l'équation de l'onde analytiquement par différentes méthodes (transformée de Laplace, séparation des variables, méthode des caractéristiques) ou numériquement. Chacun de ces méthodes est discuté par \cite{Verruijt1994}. Dans le cadre de ce travail, on retient la solution obtenue par la méthode des caractéristiques, proposé initialement par \cite{Saint-Venant1898}. Selon celle-ci, l'équation \ref{eq:onde} correspond à la superposition deux ondes $u_{f}(t-x/c_t)$ (descendante) et $u_{g}(t-x/c_t)$ (remontante) se propageant le long des tiges de manière continue à une vitesse d'ondes ($c_t$).
...
On a appliqué ce principe au modèle numérique afin de vérifier s'il est capable de reproduire la technique intégré dans l'essai DCLT. Pour ce faire, on a simulé l'essai DCLT au sein d'un massif élasto-plastique parfait représentant un sol sec. Pour cette analyse, on applique au massif représentant le sol les paramètres illustrant le sable de Fontainebleau à l'état dense : E de 150 MPa, $\nu$ de 0,33, $\phi$' de 35° et cohésion apparente de 5 kPA, $\psi$ de 5°. 
...
On présente par la suite le signaux de force et de vitesse mesurés à la proximité de l'enclume et permettant d'estimer les signaux en pointe. Les ondes descendante et remontante obtenues par découplage. Une fois les ondes découplées, on présente les signaux estimées et mesurés au niveau de la pointe. La force transmise déduite par la suite. Le signal de vitesse et de déplacement en pointe.


%\begin{figure}[htb]
%    \begin{minipage}[t]{.5\textwidth}
%        \centering
%        \includegraphics[width=\textwidth]{media/num_F_Zv.PNG}
%        \subcaption{Les ondes descendante et remontante obtenues par découplage. Une fois les ondes découplées, on présente les signaux estimées et mesurés au niveau de la pointe. La force transmise déduite par la suite. Le signal de vitesse et de déplacement en pointe.}\label{fig:1}
%    \end{minipage}
%    \hfill
%    \begin{minipage}[t]{.5\textwidth}
%        \centering
%        \includegraphics[width=\textwidth]{media/num_F_Zv.PNG}
%        \subcaption{}\label{fig:2}
%    \end{minipage}  
%    \label{fig:1-2}
%    \caption{(a) Mesures dans la tige à proximité de l'enclume (point A) : force et vitesse en fonction du temps pour un impact avec Mesures dans la tige à proximité de l'enclume (point A) : force et vitesse en fonction du temps pour un impact avec Mesures dans la tige à proximité de l'enclume (point A) : force et vitesse en fonction du temps pour un impact avec }
%\end{figure}

\begin{figure}[H]
   \begin{center}
        \includegraphics[scale =.4]{media/num_F_Zv.PNG}
        \caption{Mesures dans la tige à proximité de l'enclume (point A) : force et vitesse en fonction du temps pour un impact avec $v_{imp}$ = 1,75 m/s}
          \label{fig:num_F_Zv}
      \end{center}
\end{figure}
...
\begin{figure}[H]
   \begin{center}
        \includegraphics[scale = .4]{media/num_tech_Ft.PNG}
        \caption{Résultats des calculs pour signaux de force des ondes : onde descendante, onde remontante et onde transmise estimée (force en pointe) ; Signal de force mesuré en pointe  F(t) (Équation \ref{eq:reconstruction_F}}
          \label{fig:num_tech_Ft}
      \end{center}
\end{figure}
...
\caption{(a)}\label{fig:mouse}
\end{subfigure}


\begin{subfigure}[b]{.45\linewidth}
\includegraphics[width=\linewidth]{media/num_F_Zv.PNG}
\caption{(b)}\label{fig:mouse}
\end{subfigure}
...
Selon \cite{Ahmadi2005}, ce critère de rupture simple est capable de traduire correctement le comportement des sables. Les auteurs l’appliquent afin de modéliser le comportement d’un sable purement frottant (comme le sable de Ticino). L’angle de frottement est obtenu à partir d’une série d’essais triaxiaux tandis que l’angle de dilatance est estimé à partir de l’angle de frottement effectif $\phi$’ et l’angle de frottement à l’état critique $\phi_{cv}$.
...
L'étude bibliographique réalisée à propos des modèles d'interaction a permis de souligner leur points convergeant et leurs limitations. A l'issue de cette étude, on envisage d'appliquer un modèle d'interaction sol-pointe plus réaliste (comme à celui de \cite{Loukidis2008AssessmentProcedure}) afin de dissocier la composante dynamique et de pouvoir appliquer des modèles d'interprétation comme l'expansion de cavité à cette technique. 
